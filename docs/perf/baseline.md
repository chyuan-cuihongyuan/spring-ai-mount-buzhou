...
## effort #15 增补哨兵（T245 / impl-194；stub 嵌入口径，2026-08-17 首轮待 nightly）

| 哨兵 | 场景 | 设计预期 | 硬顶（10 倍宽幅） |
|------|------|---------|------------------|
| 语义满桶查询 | 128 条 × 384 维桶内全量 cosine 最近邻（stub 嵌入） | <1ms | 10ms |

> 口径：stub 嵌入（确定性哈希向量）——只测进程内扫描开销本体；真实嵌入调用延迟归
> embedding provider（OpenAI 等），开启语义缓存的端到端成本另计（runbook §语义缓存
> 成本口径）。跨机器绝对值不可比，只看同机趋势。
