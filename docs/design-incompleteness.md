...
1. **api/SPI Javadoc 系统性缺失**（「api 子包与 SPI 必须有 Javadoc」）：core 46 个公开类型无类型级 Javadoc，含规约自称的范式文件本身——`BuzhouHook.java:3`、`HookChain.java:13`、`AgentSession.java:6`、`AgentRuntime.java:3`、`MessageStore.java:8`、`HarnessToolCallingManager.java:31`；memory/spill 另有 43 个零 Javadoc 公开类型（✅ memory/spill 40 缺已补：spec 1529；resilience 域留候选池）。
