---
id: T1870
title: R34 补录——金丝雀候选限流放行与配额窗口语义（CanaryQuotaExhaustedTest 演化）
type: task
status: closed
assignee: zcode-k
blocked-by:
...
K 会话第 34 轮（编号补录——R28 提交时误跳 T1870；本票按跳号先例回填）：金丝雀候选级限流（candidateLimiter acquireOrThrow）的放行/耗尽两态语义如何验证？
...
3. **边界**：耗尽降级 e2e 断言因全局限流窗语义暂不成立（主路同窗亦受限），以放行态+窗口语义实证收口；按模型独立配额属 ModelRateLimiter 增强域（后续轮）。
