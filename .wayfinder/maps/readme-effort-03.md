> **历史存档**：以下为 effort #3 运行期的 tracker 约定（当时每个 effort 一个独立目录 `.wayfinder3/`）。
> 2026-08-17 起全部 effort 融合为单一 `.wayfinder/`：map → `maps/effort-03.md`，决策票 → `../tickets/`，实现切片 → `../impl/`，现行约定见 [`../README.md`](../README.md)。

# Wayfinder3 Tracker（本地 markdown · effort #3「生产级收口」）

本目录是**第三个 effort** 的 wayfinder 图，延续 [`.wayfinder/`](../README.md)（effort #1，T1–T27）与 [effort #2 约定存档](readme-effort-02.md)（effort #2，T28–T54 全闭合、27 实现纵切片落地）。**map = `MAP.md`**；**决策票 = `tickets/T<n>-<slug>.md`**（编号 **T55 起全局续用**）。
