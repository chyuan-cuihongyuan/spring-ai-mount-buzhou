...
- **阿里云镜像泄漏（双轴复审 Standards 头号项，已彻底修复）**：根 POM 原 `<repositories>`/`<pluginRepositories>`（本机绕 repo.maven.apache.org TLS 指纹拦截）会被写进已发布 POM、传染下游且 Central 拒收。曾尝试 `flatten-maven-plugin`（oss / defaults / 显式 pomElements）在发布时剥离 repositories——均异常（oss 保留 repositories 却删 dependencyManagement 致 BOM 空壳；显式 `<scm>keep</scm>` 等不生效），回退。**最终方案（用户确认）**：镜像从 POM 迁至仓库根 `settings.xml`（`<mirrors>` 把 central→阿里云），同步改 CLAUDE.md 本地构建说明（本机 `cp settings.xml ~/.m2/settings.xml`）；POM 不含 repositories，已发布 POM 干净、Central 不拒收。CI（ubuntu-latest）直连 central，无需镜像。
