...
| 端点治理 | ETag 条件请求匹配 | EntityTagMatcher——查询/导出端点版本复用与乐观并发（HTTP RFC 7232 思想）：If-None-Match 命中回 304 省全量载荷（弱比较语义等价）+If-Match 未满足 412 前置拦截他人已改（强比较字节等价）+W/ 弱 ETag 语义等价与强 ETag 区分+* 通配——与续读令牌互补（版本复用 vs 分页续读）（spec 2046） | [spec 2046](docs/spec/2046-entity-tag-matcher.md) |
...
| 并发治理 | 并查集 | DisjointSet——动态等价类传递闭包均摊近常量（Union-Find 路径减半+按秩合并思想，Tarjan α(n)）：union 声明同源（冗余合并 false 不动账面）+find 同根即连通（A=B、B=C ⇒ A=C 免 O(n²) 重扫）+componentCount 守恒读数+sizeOf 聚合+越界/容量 0 fail-fast——会话归并/重复键消解的地基件，与向量时钟（判先后）互补（本件判同源）（spec 3002） | [spec 3002](docs/spec/3002-disjoint-set.md) |
| 并发治理 | EDF 最早截止期队列 | EdfScheduler——多源时限统一截止期序（实时调度 EDF 思想：单处理器可调度最优）：截止期升序+同刻入队序 FIFO（tie-break 确定性免同刻抖动）+空态 +∞（NO_DEADLINE 比较恒安全）+headLaxity 余量负即超期可判+Pending(deadline,sequence,id) 全息记录——轮次 deadline/工具超时/排空窗「谁最先到期」免各处自扫（spec 3003） | [spec 3003](docs/spec/3003-edf-scheduler.md) |
...
| 观测计量 | Morris 近似计数器 | MorrisCounter——概率计数换空间（Robert Morris 1978 思想）：只存指数 v，增量以 2^−v 概率才 +1（计数越大越懒——写放大对数衰减），估计 2^v−1 期望无偏——O(log log n) 空间（十亿级计数 5 位指数），海量低价值基数（调用/心跳/提及数）的粗口径件+首计数必中+指数封顶 62 诚实边界+RandomGenerator 注入回放；与 HLL（去重基数）/频率素描互补（本件单流总计数）（spec 3008） | [spec 3008](docs/spec/3008-morris-counter.md) |
| 溢出治理 | ARC 自适应替换缓存 | AdaptiveReplacementCache——新近/频率四链自平衡（IBM Megiddo-Modha ARC 思想）：T1 一次轨道+T2 多次轨道+B1/B2 幽灵链（逐出键记名），幽灵命中自适应调 p（B1 命中调增——多保新面孔/B2 命中调减——多保热面孔）+REPLACE（T1 超 p 逐 T1-LRU 入 B1 否则逐 T2-LRU 入 B2）+命中一律晋升 T2——一次性扫描流只污染 T1（LRU 热键全冲刷病的根治），targetRecency/四链大小读数可对账（spec 3009） | [spec 3009](docs/spec/3009-arc-cache.md) |
...
| 策略治理 | KMP 字符串搜索 | KmpSearch——精确子串匹配线性时间（Knuth-Morris-Pratt 失配函数思想）：模式预构最长真前后缀表（lps），失配滑模式不回退主指针 O(n+m)——朴素回退最坏 O(n·m) 的根治；findAll 可重叠全命中（命中回退 lps 前缀继续——「aa」在「aaaa」中命中 0/1/2）+failureFunction 公共读数（对账/教学面）+空模式双约定（indexOf 0 JDK 同款/findAll 诚实拒绝）——敏感串/守卫规则/停用词匹配地基，多模式聚合 Aho-Corasick 留白（spec 3014） | [spec 3014](docs/spec/3014-kmp-search.md) |
| 观测计量 | Fenwick 树 | FenwickTree——点更新与前缀和双 O(log n)（Binary Indexed Tree lowbit 区间分解思想，Fenwick 1994）：槽位点 add 累加（可负）沿 lowbit 上行改 log 槽+前缀 prefixSum 沿 lowbit 下行聚合——朴素数组「更新 O(1) 查询 O(n) 或反之」两难的根治；rangeSum=前缀差+total——流式频次表（延迟分桶直方/调用计数槽）高频更新与秩/分位前缀查询的单件化地基；1000 随机点更新对拍朴素数组恒等入档（spec 3015） | [spec 3015](docs/spec/3015-fenwick-tree.md) |
...
| 策略治理 | 跳增一致性哈希 | JumpConsistentHash——分桶路由零内存+扩缩容最小迁移（Google jump hash 思想，Lamping-Masayuki 2014）：线性同余跳增（跳到 j 概率 1/(j+1)），O(1) 空间零表零虚节点；n→n+1 仅 ≈1/(n+1) 键迁移且全落新桶——朴素取模 n 变全量重排病的根治；契约边界诚实声明：只支持尾端扩缩（任意增删+权重归哈希环），与 ConsistentHashRing 成对按场景选型（零内存 vs 灵活性）（spec 3020） | [spec 3020](docs/spec/3020-jump-consistent-hash.md) |
| 观测计量 | Holt 双参数预测器 | HoltForecaster——趋势型序列在线短程外推（Charles Holt 1957 双参数指数平滑思想）：水平 α+趋势 β 双分量递推（level=αx+(1−α)(level+trend)；trend=βΔlevel+(1−β)trend），forecast(h)=level+h·trend——EWMA 只平滑水平（无趋势外推，趋势序列系统性滞后）与线性回归窗口敏感两病的补位；预算 ETA/延迟爬升/累积用量恶化的短程预测件；常量序列趋势恰零/线性序列趋势收敛真斜率的数学性质入档（spec 3021） | [spec 3021](docs/spec/3021-holt-forecaster.md) |
| 并发治理 | 二择选择器 | TwoChoiceSelector——多目标派单两问取轻（Power of Two Choices 思想，Azar/Mitzenmacher）：随机抽两相异候选取负载小者（并列先抽），O(1) 决策换最大负载 Θ(ln n/ln ln n)→Θ(ln ln n) 指数级改善（朴素单抽尾部桶过热的根治）——不扫全表不加中心队列；纯函数零状态（loads 快照归调用方派单回写）+等负载退化均匀+万球十桶对拍单抽紧界证据入档——工具端点/模型上游/工作者派单件，与权重路由/分片路由正交可叠加（spec 3022） | [spec 3022](docs/spec/3022-two-choice-selector.md) |
| 过程治理 | Q 系 R24 周期对账 | Wave 4 五新类型快照补登（1070→1075，CONTEXT 969→974——Tarjan/TTL 抖动/jump hash/Holt/二择）+ 全仓 verify 三门绿（Q 第四波）；占坑换题四例（RRF/BM25/Wilson/burn-rate 均已被占——grep 夘核纪律生效）+概率口径（相异重抽 vs 带放回）与归一化常数教训 rc 门禁提交前拦截（spec 3023） | [spec 3023](docs/spec/3023-q-r24-reconciliation.md) |
| 评估治理 | Grubbs 离群检验 | GrubbsOutlier——单点毛刺统计判定（Grubbs 1950 极端学生化偏差思想）：G=max|xᵢ−mean|/s 对内置 α=0.05 双侧临界值表（n=3..32，卡方表制先例）——高/低双侧检出+检出才报离群值（未检出诚实 null）+矩复用 WelfordAccumulator+零方差（全等样本）诚实拒绝——毛刺延迟/异常成本/跑分野值不拍阈值可判；单离群口径与 IQR 围栏互补（多离群互掩归 IQR）（spec 3024） | [spec 3024](docs/spec/3024-grubbs-outlier.md) |
| 策略治理 | Rabin-Karp 滚动搜索 | RabinKarpSearch——滑窗指纹 O(1) 滚动更新（Karp-Rabin 1987 多项式滚动哈希思想）：h'=(h−左出·B^(m−1))·B+右进免整窗重哈希（滑窗 O(n·m) 病的根治）+哈希命中逐字复核（Las Vegas——环绕假阳零误报）+hashOf 内容哈希读数（同内容同哈希，滚动与整串同口径可复算）+long 溢出环绕隐式取模（Java 环绕确定性）——流式敏感串/滑窗指纹/去重的可复用口径，与 KmpSearch 成对（失配跳跃 vs 滚动指纹各取所长）；300 随机三方对拍+千级高重复压力入档（spec 3025） | [spec 3025](docs/spec/3025-rabin-karp-search.md) |
| 策略治理 | Box-Muller 高斯采样器 | GaussianSampler——均匀源显式造高斯（Box-Muller 1958 极坐标变换思想）：u₁∈(0,1]（1−nextDouble 免 log(0)）与 u₂ 极坐标变换一次产一对独立标准正态（samplePair 天然单位——退避抖动建模/蒙特卡洛扰动/合成噪声地基）——JDK nextGaussian 黑盒无口径可审计病的显式件；无缓存口径不藏状态（纯函数性优先）+平移缩放 N(μ,σ²)（σ=0 退化常量）+RandomGenerator 注入回放；10 万样本矩收敛与 1σ 68.27%/2σ 95.45% 贴理论入档（spec 3026） | [spec 3026](docs/spec/3026-gaussian-sampler.md) |
| 并发治理 | OR-Set 观察删除集 | ObservedRemoveSet——多副本集合并发删加免协调收敛（CRDT Observed-Remove Set 思想，Shapiro 2011）：add 带唯一标签 Tag(replica,seq)，remove 只墓碑当时观察到的标签——并发新标签幸存（add 胜显式语义：一端删一端并发加不丢不复活），merge 标签并+墓碑并（幂等交换只读对侧）——会话标签/协作标注/多端收藏的集域收敛件，与 LWW 寄存器（值域）/PN-Counter（数域）成 CRDT 三形态；皇冠场景（A 加→同步→B 删→A 再加→互合同步→幸存）入档（spec 3027） | [spec 3027](docs/spec/3027-observed-remove-set.md) |
| 溢出治理 | CLOCK 二次机会驱逐 | ClockEviction——近似 LRU 的 O(1) 低摩擦档（CLOCK/second-chance 页替换经典思想）：环形帧阵列+引用位，命中/更新置位零搬移（严格 LRU 链表逐次搬移贵的对照）；满载时针扫描位 1 清位跳过（二次机会——访问近序不被一刀切，FIFO 盲逐热点病的根治）位 0 逐出——evictedCount 对账读数；与 AdaptiveReplacementCache 成驱逐两档（自适应精确 vs 低摩擦近似按维护成本选型）（spec 3028） | [spec 3028](docs/spec/3028-clock-eviction.md) |
| 过程治理 | Q 系 R30 周期对账 | Wave 5 五新类型快照补登（1075→1080，CONTEXT 974→979——Grubbs/Rabin-Karp/Box-Muller/OR-Set/CLOCK 五包落位）+ 全仓 verify 三门绿（Q 第五波；30/150=20% 五分之一里程碑）；Grubbs 表长双写与 CLOCK 进入位语义两教训均 rc 门禁提交前拦截（spec 3029） | [spec 3029](docs/spec/3029-q-r30-reconciliation.md) |
| 并发治理 | 时间轮定时器 | HashedWheelTimers——海量短定时任务 O(1) 入轮批量到期（Netty HashedWheelTimer/Kafka hashed wheel 思想）：schedule 按截止期落槽 (deadline/tick)%size（优先队列逐次比较 O(log n) 全序维护病的根治），大延迟多轮滞槽；advanceTo 时针掠槽弹到期（deadline 序+同刻 id 序确定性）——免线程纯数据结构件（tick 循环归调用方）+时间调用方传入+守恒 scheduled==fired+pending+粒度诚实边界（槽重访校验——deadline 后最多再等一个轮周长 wheelSize×tick 显式可诺）；与 EdfScheduler 互补（取最优队列 vs 批量到期结构）（spec 3030） | [spec 3030](docs/spec/3030-hashed-wheel-timers.md) |
| 并发治理 | vruntime 公平队列 | VirtualRuntimeQueue——异质实体按权重动态公平分吞吐（Linux CFS vruntime 思想）：虚拟运行时间按权折算（vruntime += work/weight——重权者慢走），恒取最小者（TreeMap O(log n) 红黑树同构）——等权退化轮转、三倍权三倍配额（千择 750/250 定量证据），时间片轮转静态分片僵化的根治；并列先入先选确定性可回放+账面全读可审计+新注册从 0 起追赶份额诚实口径声明（CFS min_vruntime 对齐留白）——工具配额/租户公平/会话资源份额原语，与 WeightedFairScheduler（DRR 整数赤字）成公平双档（spec 3031） | [spec 3031](docs/spec/3031-virtual-runtime-queue.md) |
| 策略治理 | Maglev 哈希 | MaglevHash——查表 O(1) 常数+最小扰动+可控分布（Google Maglev LB 2016 置换表思想）：每节点步进置换 (offset+j·skip) mod M 轮询抢占查找表空位——均匀（份额差 ≤1 槽）且增删节点仅 ~1/n 键迁移（千键 3→4 节点迁移率 ~1/4 定量证据——朴素取模 ~3/4 重排病的根治）；候选重复即权重（占 ~k/n 槽）+表大小素数校验（置换满周期前提）+种子复用 DeterministicHash 确定性+entriesOf 份额对账面——与哈希环（任意增删）/jump（零内存）成路由三件按运维形态选型（spec 3032） | [spec 3032](docs/spec/3032-maglev-hash.md) |
| 溢出治理 | 2Q 双队列缓存 | TwoQueueCache——扫描抗性静态分诊档（Johnson-Shasha 1994 Two-Queue 简化版思想）：入口 A1in FIFO 观察窗+主区 Am LRU，二触晋升（「热不热看第二触」）——一次性扫描流只在观察窗自旋（LRU 全 cache 冲刷病的另一解），静态比分简于 ARC 幽灵自适应——与 AdaptiveReplacementCache/ClockEviction 成驱逐三档（自适应/静态分诊/零分诊）覆盖维护成本谱系；双逐出计数分账可审计（spec 3033） | [spec 3033](docs/spec/3033-two-queue-cache.md) |
| 观测计量 | Croston 间歇预测器 | CrostonForecaster——稀疏零海序列偶发需求预测（Croston 1972 思想）：需求大小 z 与需求间隔 p 分开平滑（仅非零观测更新——朴素指数平滑被零海拖向零、偶发毛刺完全淹没病的根治），预测率=z'/p'——「毛刺多大不重要，率才是口径」（形状不变性：4 单@2 期与 8 单@4 期同率 ~2）——工具调用/审计事件/故障上报等间歇流的每周期期望量件；全零 NaN 诚实+双分量与稀疏度全读数——与 EWMA（连续水平）/Holt（连续趋势）成预测三件按序列形态选型（spec 3034） | [spec 3034](docs/spec/3034-croston-forecaster.md) |
| 过程治理 | Q 系 R36 周期对账 | Wave 6 五新类型快照补登（1080→1085，CONTEXT 979→984——时间轮/vruntime/Maglev/2Q/Croston）+ 全仓 verify 三门绿（Q 第六波；36/150=24%）；占坑换题四例（基数限制/负缓存/Misra-Gries/HRW 均已被占）——grep 夘核纪律六波连续生效（spec 3035） | [spec 3035](docs/spec/3035-q-r36-reconciliation.md) |
| 溢出治理 | Elias gamma 编解码 | EliasGammaCodec——幂律整数流紧凑编码（Elias 1975 前缀码思想）：N=2^k+r 编为 k 零+边界 1+k 位余数——小数 1 位起步（1→"1"/2→"010"/8→"0001000"，定长 32 位对照），零计数自定界免解码表；decode 整串恰一码字（残留位 fail-fast）+decodeAt 游标流式推进（Decoded(value,nextOffset)——免定界符连解）+bitLength 读数——事件计数/频次/差分增量传输地基，与 varint（字节界）互补按介质粒度选型（spec 3036） | [spec 3036](docs/spec/3036-elias-gamma-codec.md) |
| 观测计量 | lag-k 自相关 | Autocorrelation——序列周期/趋势/白噪一把尺（Box-Jenkins ACF 思想）：x 与滞后 k 平移的 Pearson 相关——周期自锁定 +1（lag=周期）/半周期反相 −1/趋势惯性小 lag 高正/白噪各 lag ~0（无记忆）——「输出是否打转」「指标是否有惯性」「扰动是否独立」三用一尺；零方差 NaN 诚实——与卡方（分布判定）/Grubbs（单点判定）成序列三尺（spec 3037） | [spec 3037](docs/spec/3037-autocorrelation.md) |
| 策略治理 | top-k+温度采样 | TopKSampler——候选宽度与集内锐度正交双旋钮（LLM top-k 截断×temperature 缩放思想）：前 k 名固定宽度截断（logit 降序同值小索引先——确定性）+温度缩放稳定 softmax——k 管宽度 T 管锐度四象限（k=1 恒 argmax/T→0 贪心/T 大趋均匀/截断集外恒零）——与 NucleusSampler（自适应宽度 p）成对互补各有盲区；probabilities 读数可审计+−∞ 零概率永不中（spec 3038） | [spec 3038](docs/spec/3038-top-k-sampler.md) |
| 溢出治理 | varint 编解码 | VarintCodec——字节流整数紧凑+有符号不顶格（Protobuf zigzag+LEB128 思想）：有符号 long 先 zigzag 交错映射无符号（0→0/−1→1/1→2——负小值 1 字节起步，直接 LEB128 负数 10 字节顶格病的根治），再 7 位/字节小端续位自界定（小值 1 字节、值域每 7 位加一字节：300→2/2⁴⁰→6/极值 10）——键值对/计数器/时间戳等小值占多数的字节流地基；decodeAt 游标连解+截断/残留/超宽 fail-fast——与 EliasGamma（位级前缀）成编码双件按介质粒度选型（spec 3039） | [spec 3039](docs/spec/3039-varint-codec.md) |
| 并发治理 | 时间戳锁仲裁器 | TimestampLockArbiter——资源互锁事前预防（Rosenkrantz 1978 wait-die/wound-wait 思想）：年长者 (timestamp,txnId) 字典序特权——wait-die 老等少死（老者 WAIT/少者 REQUESTER_ABORTS 自杀重启）、wound-wait 老伤少等（老者 GRANTED 伤持有者夺锁/少者 WAIT），两模式等待图恒无环（死锁预防而非检测）；纯仲裁无阻塞（等待/重启动作归调用方）+woundedTxn 指认受害者+同 txn 幂等+同钟 txnId 决——与 TarjanSccFinder 成对（事前预防 vs 事后指认）防线互补（spec 3040） | [spec 3040](docs/spec/3040-timestamp-lock-arbiter.md) |
| 过程治理 | R 系 R1 对账门落位 | RSession4000LedgerAuditTest 四面互证（spec 4000–4049 ↔ README 行 ↔ T6001+2(N−4000) 票对 ↔ impl 2101+(N−4000)，起点 4000 严格递增；Q 系公式族第五应用）+ 4000 系总图开图（50 轮号段离线实查本地全档后声明；Q/P 雾区候选静脉不抢、Q 余量号段不占）（spec 4000） | [spec 4000](docs/spec/4000-r-series-ledger-audit.md) |
| 观测计量 | Count-Min 素材计数 | CountMinSketch——固定内存任意键点查计数近似（Cormode-Muthukrishnan 2005；Cassandra/ClickHouse 内建思想）：d 行 × w 列计数矩阵每行独立散列落格，increment 全行加记、estimate 取行最小——非负增量下只高估不低估（单侧误差，ε·N 概率界由 d=⌈ln(1/δ)⌉/w=⌈e/ε⌉ 配平），「每键一个 long 的内存在基数爆炸场景不可持续」病（工具签名/错误签名/查询词频）的 O(d·w) 固定内存解；totalCount 精确守恒账 + DeterministicHash 行黄金比步进行散列确定性可回放——与 MisraGriesSketch 成对（要名单 vs 要点查）、与 HllCardinalitySketch 互补（去重基数 vs 带重计数）（spec 4001） | [spec 4001](docs/spec/4001-count-min-sketch.md) |
| 观测计量 | Space-Saving 频繁项 | SpaceSavingTopK——固定 k 槽精确计数频繁项（Metwally 2005；ClickHouse sumKMorQ 思想）：在榜键精确 +1、满榜新键淘汰最小计数者并继承其计数 +1（最小计数即误差界），保留者计数恒 ≥ 真值（单侧高估）——MisraGries 只给「计数 ≤ 真值」候选（名单之外还要名次读数需二次精扫）病的读数即答案解；top() 计数降序并列先入榜先（确定性可回放）+ minCount 误差界读数 + totalObservations 守恒账——与 MisraGries（要候选）/CountMinSketch（要任意键点查）成素描三件按问句选型（spec 4002） | [spec 4002](docs/spec/4002-space-saving-topk.md) |
| 观测计量 | Boyer-Moore 多数表决 | BoyerMooreMajority——超半数元素单遍常数内存判定（Boyer-Moore MJRTY 1991 配对抵消思想）：候选 + 计数器两变量，同候选取证 +1、异候选取消 −1、归零换候选——真多数（> n/2）必幸存（每消一对至多废一张多数票），但幸存者未必多数故二次核验定夺；恰半是僵局不是多数（投票语义）——「排序取中位数 O(n log n)/哈希计数 O(n) 内存」病的单遍解，灰度版本一致性/评估标注一致性/错误签名主导性天然合用；空表/无多数诚实 null——与 MisraGries 成对（k=1 特例确定性精确版）（spec 4003） | [spec 4003](docs/spec/4003-boyer-moore-majority.md) |
| 观测计量 | KS 两样本检验 | KsTwoSample——不假设形状不分桶的两样本全分布对比（Kolmogorov 1933/Smirnov 1948；scipy ks_2samp 思想）：双样本 ECDF 最大竖直距离 D=sup|F₁(x)−F₂(x)|（排序双指针扫，tie 同步双推进），p 走渐近 Kolmogorov 分布 Q(λ)=2Σ(−1)^(k−1)e^(−2k²λ²)（Numerical Recipes 小样本校正 en=√ne+0.12+0.11/√ne；λ<0.4 渐近式失效区诚实返回 1「无法拒绝」）——t 检验只比均值（形状漂移盲区）与卡差要分桶（桶宽主观）两病的免设一尺，A/B 两版回答质量分布/两模型延迟分布/两池错误率分布同源判定件；与 WilsonInterval（单比例）/ChiSquareUniformity（均匀性）成统计三尺（spec 4004） | [spec 4004](docs/spec/4004-ks-two-sample.md) |
| 过程治理 | R 系 R6 周期对账 | Wave 1 四新类型快照补登（1129→1133：Count-Min 素材/Space-Saving 频繁项/Boyer-Moore 多数表决/KS 两样本——metrics×3+eval×1 两段；CONTEXT 计数历史欠账 984→1133 一并对齐真值）+ 全仓 16 模块离线 verify 三门绿（R 第一波——对账门六号四件套齐整）+ 6/50=12% 里程碑（spec 4005） | [spec 4005](docs/spec/4005-r-r06-reconciliation.md) |
| 溢出治理 | Huffman 前缀码 | HuffmanCodec——倾斜符号流最优前缀编码（Huffman 1952；zlib/deflate 规范码思想）：堆合并建树取码长（freq 并列按符号序确定性、单符号退化 1 位、树深 >57 fail-fast），码字按 (码长,符号) 字典序规范推导——码本只存码长不随树走（deflate 同款），firstCode 逐长左移推进；encode 位打包 MSB 先 + decode 逐位对表（码长内首码偏移命中即出符号，截断/坏码 fail-fast）——定长 8 位/字节对工具签名/文本骨架等倾斜流的等概率浪费病解；Encoded(bytes,bitCount) 记录面 + codeLength/distinctSymbols 读数——与 EliasGamma（幂律整数无表）/Varint（字节界自界定）成编码三档按符号分布选型（spec 4006） | [spec 4006](docs/spec/4006-huffman-codec.md) |
| 溢出治理 | CRC-32C 校验 | Crc32C——传输/落盘完整性软件基准校验（Castagnoli 多项式 0x1EDC6F41；iSCSI/ext4/LevelDB/SSE4.2 同款思想）：反射算法逐字节查表（init/xorout 全 1），突发错误检测强度优于 IEEE CRC-32（关键汉明距更优）且现代 CPU 有硬件加速路径——本件为软件基准口径（同输入同输出跨平台可对账，硬件路径等价性锚点）；compute 全量/区间双面 + verify 到达校验（相等即真）——与编码件（Varint/EliasGamma/Huffman）成对：编码管紧凑、校验管到达一致（spec 4007） | [spec 4007](docs/spec/4007-crc32c.md) |
| 溢出治理 | 增量基准帧编码 | DeltaFrameOfReference——列存排序块压缩（Parquet DELTA_BINARY_PACKED/ORC 思想）：每帧记基准值（frame of reference）原样、帧内余值只记与前一值的增量（delta），增量与基准全走 zigzag varint（复用 VarintCodec）——时间戳/序号/偏移等单调（近单调）流「绝对值大、差值小」病的字节面 1/3 以下解（千毫秒时间戳例证）；帧界即基准重置点——大幅跳变（分块边界/重启点）不被长程增量拖累只付一次基准代价；decode count 驱动逐帧推进（截断/残留 fail-fast）+frameSize=1 全基准退化档——与 VarintCodec 成对复用、与 EliasGamma/Huffman 成编码四档（spec 4008） | [spec 4008](docs/spec/4008-delta-frame-of-reference.md) |
| 溢出治理 | Crockford Base32 | CrockfordBase32——人类可转录整数编码（Douglas Crockford 2001；ULID 字母表同源思想）：32 符号字母表剔除 I/L/O/U（形近 I↔1、L↔1、O↔0、U 不雅词），解码侧宽容归一（O→0、I/L→1、大小写不敏感、连字符忽略）——电话/工单口传 ID 的「念错写错」病从字母表根除；可选 mod-37 校验符号（37 素数 >32，单字符检错——转写错一位必被捕获，encodeWithCheck/decodeWithCheck）+ 非法符/溢出 fail-fast——与 SnowflakeIdDecompose（位布局）正交：本件管人面转录（spec 4009） | [spec 4009](docs/spec/4009-crockford-base32.md) |
| 并发治理 | UUIDv7 时间有序生成器 | UuidV7Monotonic——字典序即时间序的零协调 ID（RFC 9562 思想）：48 位 Unix 毫秒置最高位——索引插入点局部性（v4 均匀随机把 B 树插成随机写病的根治）+ rand_a 12 位单调计数器（同毫秒批量不撞序：新毫秒随机重置/同毫秒 +1）+ 溢出向时间戳借位（unix_ts_ms+1 伪时序推进，RFC 允许——有序性不断）+ variant 10 + rand_b 62 位随机；时钟/随机源可注入（确定性回放）+ timestampOf/counterOf 回读（version≠7 拒判）——与 SnowflakeIdDecompose（协调位布局）成对：v7 零协调以时间戳换排序性（spec 4010） | [spec 4010](docs/spec/4010-uuid-v7-monotonic.md) |
| 过程治理 | R 系 R12 周期对账 | Wave 2 五新类型快照补登（1133→1138：Huffman 前缀码/CRC-32C/增量基准帧/Crockford Base32/UUIDv7——message×4+concurrent×1 两段）+ 全仓 16 模块离线 verify 三门绿（R 第二波——对账门十二号四件套齐整）+ 12/50=24% 里程碑（spec 4011） | [spec 4011](docs/spec/4011-r-r12-reconciliation.md) |
| 清理治理 | 区块 min/max 剪枝 | ZoneMapPruner——分块查询统计跳读（DuckDB zone map / Parquet row group 统计 / ORC min-max 索引思想）：每块记 (min,max,nullCount) 三统计，查询先做谓词裁剪——区间不交（min>high ∨ max<low）即整块免读，O(块数) 判定换 O(块体积) IO；边界语义双侧含等（擦边块保守读——漏读即错读）；zonesOverlapping/zonesMatching/zonesWithNulls 三裁决面 + zonesSkipped/pruningRatio 剪枝账（局部性好的列 >2/3 免读可量化，无序数据退化为 0 的诚实面）；构造期校验（min≤max/nullCount≥0/id 非空）fail-fast——与 SweepLineIntervals 正交（读不读 vs 叠几层）（spec 4012） | [spec 4012](docs/spec/4012-zone-map-pruner.md) |
| 清理治理 | 尺寸分层合并挑选 | SizeTieredMergePicker——LSM 压实成组裁决（Cassandra SizeTieredCompactionStrategy 思想）：候选按尺寸升序、均值 ±radius（1.5）内归同桶，桶员 ≥minThreshold（默认 4）才成组、成组取 maxThreshold 个最小者（单次压实写放大封顶），多桶竞选取员最多者（小文件优先清，先遇并列保持确定性）——「新旧混压」（巨块反复重写）与「碎片堆积」（小块永不合并）两病的折中件；纯裁决无 IO（压实动作归调用方）+ 输出尺寸升序并列 id 序 + 无合格组空表诚实——与 ChunkCompressionPolicy（块龄阈值）成压实双档按数据形态选型（spec 4013） | [spec 4013](docs/spec/4013-size-tiered-merge.md) |
| 清理治理 | Bitcask 键目录合并 | BitcaskKeydir——追加日志存储的死区回收账（Riak bitcask 思想）：写只追加（append-only——顺序写零放大）、读走内存 keydir（key→最新 offset/size），旧版本与删除留下死字节——totalBytes/deadBytes/deadRatio 三账面（空日志 NaN 诚实）+ shouldMerge 死比门（死比超阈才值得重写——写放大换空间的调度杆）+ mergePlan 活键/活字节成本账 + applyMerge 压实（死账清零、总账对齐活账、键目录不动）；纯账面模型（真 IO 归存储层）——与 SizeTieredMergePicker（尺寸成组）互补、与 KeyCompaction 同族不同层（spec 4014） | [spec 4014](docs/spec/4014-bitcask-keydir.md) |
| 容量治理 | Slab 类装箱 | SlabClassPacker——固定尺寸槽分档容量规划（Memcached slab allocator 思想）：块尺寸按增长因子（默认 1.25）几何级数分档（chunkSizeMin 起、maxItemSize 封顶停档），item 归最小容纳档（恰界归本档不进位）——同档等尺寸切槽，分配/释放永不产生外部碎片（「malloc 混尺寸长跑碎片化」病的根治件），代价是档内内部浪费（chunk−item，wasteRatio 可审计）；超最大块 −1 拒收 + per-class 分配记账（allocate/allocationsIn）供容量审计——缓冲池/对象池/消息帧池的地基件，与 BucketTableSizing 同族不同面（表容 vs 槽档）（spec 4015） | [spec 4015](docs/spec/4015-slab-class-packer.md) |
| 记忆治理 | HNSW 贪心层搜索 | HnswBeamSearch——稠密向量近似最近邻分层图（Malkov-Yashunin 2016；hnswlib/faiss 思想）：节点按指数分布随机落层（高层稀疏长边跳大局、低层稠密短边精定位），查询自顶向下贪心下降到第 1 层、第 0 层 beam 搜索（ef 宽度候选池——比纯贪心不困于局部最优）；插入逐层 beam（efConstruction）连 M 近邻双向边（对端超限按距离裁远端）——暴力 kNN O(n·d) 全扫病在语义记忆/去重检索的规模场景不可持续，本件以对数级跳数换近似召回（百点网格 top1 精确/top5 全召回例证）；k 超规模返全量 + 重复 id/维度不一致 fail-fast——与 MinHashSketch 互补（稠密向量近邻 vs 集合相似度）（spec 4016） | [spec 4016](docs/spec/4016-hnsw-beam-search.md) |
| 过程治理 | R 系 R18 周期对账 | Wave 3 五新类型快照补登（1138→1143：区块剪枝/尺寸分层合并/Bitcask 键目录/Slab 装箱/HNSW——cleanup×3+cache×1+memory×1 三段）+ 全仓 16 模块离线 verify 三门绿（R 第三波——对账门十八号四件套齐整；排除一个既有 flaky：ShadowMirrorEndToEndTest 异步镜像竞态单测绿全跑偶红；途中清理 8 天龄僵尸 surefire fork）+ 18/50=36% 里程碑（spec 4017） | [spec 4017](docs/spec/4017-r-r18-reconciliation.md) |
| 背压治理 | CoDel 受控延迟 | CoDelController——sojourn 时间驱动的 AQM（Nichols-Jacobson 2012/RFC 8290；Linux fq_codel 思想）：只盯驻留时间（入队→出队时延）是否超目标（target 5ms 级）——超目标持续一个 interval（100ms 级）进入丢包态首丢，此后间隔按 interval/√n 递缩持续早丢，回到目标下立即复位——「bufferbloat：队列满才丢、尾丢成批同步」病的时间维度解（丢不丢看延迟不看水位，早丢小量打散同步突发，低速链路/变速流量不受水位口径误伤）；时钟可注入（确定性回放）+ Queue 便捷门面（offer/poll 陈头丢新头过 + dropped/passed/depth 账面）——与 RandomEarlyDrop（RED 水位概率）成 AQM 双档（spec 4018） | [spec 4018](docs/spec/4018-codel-controller.md) |
| 背压治理 | QUIC 反放大窗 | QuicAmplificationWindow——未验证对端防反射放大闸（QUIC RFC 9000 §8.1 思想）：对端地址未验证（未证明能收其回包）前，发送量不得超已接收量的 k 倍（k=3）——信用窗模型（onReceived 入账 ×factor / onSent 扣减 / 超信用 fail-fast 的 MUST NOT 放大语义），防被诱骗当反射器打第三方（IP 伪造源 DDoS 面）；初始授信覆盖握手首包（未收任何包也要能回）+ validateAddress 验证通过即解除窗（此后全速、credit 读 Long.MAX_VALUE 语义面）——与 HierarchicalTokenBucket（稳态限速）互补：本件是未验证对端的临时放大闸（spec 4019） | [spec 4019](docs/spec/4019-quic-amplification-window.md) |
| 并发治理 | tokio 协作预算 | CoopBudget——异步任务霸占 worker 的运行时级根治（Tokio coop budget 思想）：任务每次被调度获得固定 N 点预算，关键路径操作 charge 扣 1、预算尽 hasBudget 翻假（让出时机）、yield 重置满额（重新排队后继续）——让出点由关键操作自然分布（业务代码无需自觉 yield，单任务长循环不让出饿死同 worker 其他任务的病从运行时根除）；尽后再扣 fail-fast（让出检查缺失即红）+ 每任务独立账户 + initial/remaining 读数可审计——纯预算账（真调度归运行时）；与 SpawnGate（准入）互补：彼管「进不进」、本管「进了之后霸不霸」（spec 4020） | [spec 4020](docs/spec/4020-coop-budget.md) |
| 并发治理 | 工作窃取对半分割 | WorkStealingSplit——存量负载失衡的搬运裁决（Cilk THE 双端队列 + Go scheduler/Ray/Dask steal-half 思想）：owner 一端 LIFO 热取（缓存友好——最新最深任务先跑），闲工从另一端冷偷（最老任务——依赖浅可独立展开），偷取量对半（victimSize/2、下限 minSteal、上限 victimSize、空 0）——一次摊平梯度减少偷取风暴往返；stealCount 定量 + stealFrom 冷端搬运两面（纯裁决件，真线程/真双端队列并发归运行时）——与 TwoChoiceSelector（到达时刻择短）互补：新任务去哪 vs 存量失衡怎么搬（spec 4021） | [spec 4021](docs/spec/4021-work-stealing-split.md) |
| 观测治理 | 尾采样策略 | TailSamplingPolicy——完整轨迹再裁决的采样件（OpenTelemetry tail-based sampling；Jaeger adaptive 同族思想）：头采样在 span 起点盲抽（错误/慢轨迹与正常轨迹同概率被丢——排障金料恰最易丢），尾采样等整条轨迹完成再判——错误必采（金料通道不受预算影响）+ 慢必采（时延 ≥ 阈值含等）+ 其余概率基线且受采样预算封顶（预算尽 budget 丢弃——观测成本有硬顶而金料不丢）；Verdict(sampled, reason) 五理由面（error/slow/probabilistic/budget/below-threshold）+ 四计数守恒账 + 随机源可注入确定性回放——与头采样（起点一致性传播）成双档（spec 4022） | [spec 4022](docs/spec/4022-tail-sampling-policy.md) |
| 过程治理 | R 系 R24 周期对账 | Wave 4 五新类型快照补登（1143→1148：CoDel/QUIC 反放大/协作预算/工作窃取/尾采样——backpressure×2+concurrent×2+observability×1 三段）+ 全仓 16 模块离线 verify 三门绿（R 第四波——对账门廿四号四件套齐整，排除 R18 已入档 ShadowMirror flaky）+ 24/50=48% 里程碑（spec 4023） | [spec 4023](docs/spec/4023-r-r24-reconciliation.md) |
| 策略治理 | 拓扑约束放置 | TopologySpreadPlacer——跨域容量散布硬约束（K8s TopologySpreadConstraints maxSkew 思想）：每域记现有数，放置看放置后全域斜度（max−min）≤ maxSkew 才可放——拥挤域自动截止、宽松域可进（「都堆热域」病的静态硬约束解）；eligibleDomains 候选序按现有数升序、并列域名字典序（先填最空，确定性可回放）+ skewOf 斜度读数（空图 0 诚实）+ canPlace 单域裁决——与 ConsistentHashRing 正交（容量面 vs 键空间面）；纯裁决无调度（spec 4024） | [spec 4024](docs/spec/4024-topology-spread-placer.md) |
| 跑飞防护 | 监督者重启强度 | SupervisorRestartIntensity——重启风暴累积判定与升级（Erlang/OTP supervisor max_restarts/max_intensity 思想）：滑窗内至多 maxRestarts 次子重启，越限即闩锁升级（OTP 语义：终止全部子进程并停监督者——「无限重启循环把故障刷成噪音」病的熔断升级），闩锁保持到显式 reset（重启的监督者开新纪元从头计）；时钟可注入（确定性回放）+ restartsInWindow 滑窗读数（先淘汰再读，可审计）——与 TurnStallWatchdog（单轮失速）互补：本件管重启频率累积（spec 4025） | [spec 4025](docs/spec/4025-supervisor-restart-intensity.md) |
| 执行治理 | 推测执行裁决 | SpeculativeStragglerPolicy——机器性 straggler 的副本竞争裁决（Spark speculative execution 思想）：同阶段任务显著慢于同伴中位 ×multiplier（用中位非均值——抗离群完成者拉偏）且进度仍落后 threshold（0.75 级——快完成的慢任务豁免，副本大概率赶不上白烧资源）才启副本竞争先到先胜——尾部等待被长尾任务的机器性慢（非数据性慢）拖死的病解；medianElapsed 偶数取下中位（确定性）+ TaskStats(id,elapsed,progress) 读数面——与 Hedged requests（网络对冲族）同思想不同面：请求级延迟对冲 vs 任务级进度中位对比（spec 4026） | [spec 4026](docs/spec/4026-speculative-straggler.md) |
| 策略治理 | 稳定匹配 | StableMatching——双边偏好无阻塞对配对（Gale-Shapley 1962 延迟接受；NRMP 实配算法思想）：求婚方按序出价、受婚方持有当前最优（按其偏好序）并拒绝余者，被拒者继续下一选——有限步收敛到稳定匹配（不存在双方都更愿意离开现配的阻塞对），结果对求婚方最优（主动方优势）；贪婪逐最大匹配会产出阻塞对（双方私下都更愿意换）病的根治——模型-路由偏好对接/agent-槽位分派的双边「稳定」语义件；自由队列按入参序（确定性可回放）+ 不齐边与链尽者诚实不配——与 TopologicalSorter（偏序排程）同族不同问（顺序 vs 配对）（spec 4027） | [spec 4027](docs/spec/4027-stable-matching.md) |
| 策略治理 | 动态 snitch 惩罚 | DynamicSnitchPenalty——副本延迟持续观测动态降权（Cassandra DynamicEndpointSnitch badness 思想）：每副本 EWMA 平滑延迟（α=0.5——抖动不惊弓），显著慢于最快副本 ×threshold 者加固定罚分——ranking 把病副本推队尾（读修复/一致性读优先健康副本），恢复后 EWMA 回落自动免罚复用（不手工摘副本）；未见副本零知识不罚（NaN 诚实，不入 ranking）——静态偏好不追故障病的持续观测动态偏好面；与 TwoChoiceSelector（单次两问取轻）互补：持续观测 vs 单次选择（spec 4028） | [spec 4028](docs/spec/4028-dynamic-snitch-penalty.md) |
| 过程治理 | R 系 R30 周期对账 | Wave 5 五新类型快照补登（1148→1153：拓扑散布/重启强度/推测执行/稳定匹配/动态 snitch——policy×4+runaway×1+exec×1 三段）+ 全仓 16 模块离线 verify 三门绿（R 第五波——对账门三十号四件套齐整，排除 R18 已入档 flaky）+ 30/50=60% 里程碑（spec 4029） | [spec 4029](docs/spec/4029-r-r30-reconciliation.md) |
| 定价治理 | EIP-1559 基础费调节 | Eip1559BaseFee——配额定价的有界供需调节（EIP-1559 弹性费用市场/go-ethereum calcBaseFee 思想）：每轮按用量 vs 目标调节基础费（满块 2×target 涨、空块跌、恰目标不变），弹性钳制下单步幅度天然 ≤ ±1/8——有界不尖叫；跌到地板 minBaseFee 止跌；BigInteger floor division 与 go-ethereum Euclidean division 同口径（下跌方向不舍入回零）；超弹性（用量 > 2×target）fail-fast——机器性约束由调用方 gas limit 保证，越界即违约上抛不静默钳制；与 ModelRateLimiter（令牌桶硬闸门）互补：费用市场软调节 vs 速率硬闸（spec 4030） | [spec 4030](docs/spec/4030-eip1559-base-fee.md) |
| 定价治理 | 难度目标重定 | DifficultyRetarget——难度目标的周期性有界重定（Bitcoin difficulty retarget 2016 块窗 ±4× 钳制思想）：每窗按窗内实际耗时 vs 目标耗时重定目标，实际耗时钳到 [target/4, ×4]——单窗调整幅度天然 ≤ 4× 不尖叫（快 2× 真比例、慢 8× 钳 4×）；powLimit 封顶（目标上限=难度地板）；重定块自身开启新窗滚动持续运行；非递减时间戳倒流 fail-fast（确定性）+ BigInteger 精确——工作负载难度对算力漂移「不重定失锚/自由重定尖叫」两端病的根治；与 Eip1559BaseFee 同族不同面：每轮小幅软调节 vs 周期窗钳制硬重定（spec 4031） | [spec 4031](docs/spec/4031-difficulty-retarget.md) |
| 定价治理 | 祖先费率打包 | AncestorFeerate——依赖图任务的祖先包聚合价值打包（Bitcoin CPFP Child-Pays-For Parent 思想）：排名按祖先闭包（含自身）∑fee/∑size，inclusionOrder 每轮取全池祖先费率最高者、其闭包**整包入场**（父先子后）——高费子把低费父拖进批次（贪婪单笔排序让低费父永远滞留、高费子的依赖未满足谁也进不了批次的病解）；共享祖先去重（多子不重复抬）；父自身包费率诚实不含子孙；前置先注册 DAG-by-construction（未知父 fail-fast 即防线）+ DFS 环检测留防御 + 费率并列 id 字典序确定性——与 TopologicalSorter 同族不同问（偏序排程 vs 聚合价值打包序）（spec 4032） | [spec 4032](docs/spec/4032-ancestor-feerate.md) |
| 定价治理 | 语义化版本序 | SemVerOrder——版本协商的机器性优先级（Semantic Versioning 2.0.0 §11 思想）：三段数值序 + pre-release 低于正式版（1.0.0-alpha < 1.0.0）+ 标识符逐段比（纯数字段无溢出数值比、含字母段 ASCII 字典序、数字段 < 字母段、前缀全等字段多者高）+ build 元数据不参与优先级——字符串字典序排版本的病解（1.0.10 字典序 < 1.0.9、-rc 被排到正式版之后）；parse 全量语法 fail-fast（数字段禁前导零等 §2 规则）；嵌套 Version record 不另立面——与 CrockfordBase32 同族不同面：标识编码 vs 标识排序（spec 4033） | [spec 4033](docs/spec/4033-semver-order.md) |
| 过程治理 | R 系 R36 周期对账 | Wave 6 五新类型快照补登（1153→1158：EIP-1559 基础费/难度重定/祖先费率/语义化版本序/JSON Patch——policy×5 一包）+ 全仓 16 模块 verify 三门绿（R 第六波——对账门卅六号四件套齐整）+ 36/50=72% 里程碑（spec 4035） | [spec 4035](docs/spec/4035-r-r36-reconciliation.md) |
| 定价治理 | JSON Patch 应用 | JsonPatchApplier——配置/状态文档的标准增量操作面（RFC 6902 JSON Patch + RFC 6901 JSON Pointer 思想）：add/remove/replace/move/copy/test 六操作全量（§4 语义逐条），指针 "" 根、~0/~1 转义、数组数字下标、add 的 - 追加语义；原子性（深拷贝上应用，任一操作失败整 patch 拒带操作序号，原文档不变）+ move 数组下标记账（先删后插移位诚实处理）+ test 失败即整 patch 失败（条件前置语义）——全量覆盖（大文档重传、并发互踩）与自造 diff 语义（无标准、不可审计）的病解；与增量+基准帧编码（spec 4008）同族不同面：文档操作增量 vs 数值编码增量（spec 4034） | [spec 4034](docs/spec/4034-json-patch-applier.md) |
| 指标治理 | 标量卡尔曼滤波 | ScalarKalmanFilter——单指标带不确定性闭环融合（标量 Kalman 预测/更新两步思想）：预测步常值模型外推不变、方差增长 p+=q（过程噪声累积不确定性），更新步增益 k=p/(p+r) 融合收缩（x+=k(z−x)、p*=1−k）——增益随确定性升高自适应下降（愈发不信新读数）、predict 后方差回升重新信任读数，闭环自适应；读数 state/variance/lastGain；裸读数噪声直用（抖动惊弓）或朴素滑动平均（滞后死板无不确定性度量）的病解；与 EWMA（DynamicSnitchPenalty α 平滑）同族不同面：恒定权重 vs 增益随方差闭环自适应（spec 4036） | [spec 4036](docs/spec/4036-scalar-kalman.md) |
| 指标治理 | Holt-Winters 季节指数 | HoltWintersIndex——周期负载的在线季节项学习（statsmodels ExponentialSmoothing 加法三参数思想）：水平/趋势/季节三项在线更新（L=α(y−S_{t−m})+(1−α)(L+b)、b=β(L−L′)+(1−β)b、S_t=γ(y−L)+(1−γ)S_{t−m}）+ statsmodels 式恰两季预热启发式（首季均值定水平、季差定趋势、首季去均值定季节初值）+ forecast(h)=L+h·b+S[(n+h) mod m] 与 seasonalIndexOf 读数；预热未毕业诚实 ISE 不猜——无季节项外推在周期信号上系统性高估/低估（预测永远追不上节律）的病解；与 HoltForecaster 同族不同面：水平+趋势 vs 水平+趋势+季节指数（spec 4037） | [spec 4037](docs/spec/4037-holtwinters-index.md) |
| 指标治理 | Theil-Sen 稳健斜率 | TheilSenSlope——趋势估计的离群免疫（Theil–Sen 成对斜率中位数/scipy theilslopes 思想）：斜率=全部成对斜率（竖直对跳过）的中位数——崩溃点 ≈29%（近三成点被污染仍稳），截距=median(y−slope·x)（Sen 口径）；中位数偶数取下中位（确定性，SpeculativeStraggler 同款约定）+ 纯函数 fit/predict + 竖直退化/不齐/n<2/非有限 fail-fast——最小二乘对离群点零抗性（单点杠杆拉偏整条线：+1000 污染点把 OLS 斜率拉到 108 倍 while TS 纹丝不动 1.0）的病解；与 Grubbs（Q 系离群检验）互补：检验剔除 vs 估计免疫（spec 4038） | [spec 4038](docs/spec/4038-theilsen-slope.md) |
| 指标治理 | DDSketch 相对误差分位 | DdSketch——任意量级同精度的分位草图（Datadog DDSketch 对数桶思想）：γ=(1+α)/(1−α)，桶序 idx=⌈ln v/ln γ⌉——桶宽相对恒定（µs 级与 s 级延迟同相对精度），分位估计=γ^idx（相对高估 <γ 论文诚实口径）+ min/max 精确旁路 + merge 同 γ 桶计数相加（多实例聚合成全局分位，误差保证不变）+ 空草图 ISE/正值域/越界 fail-fast——全量存储内存爆炸与固定绝对精度直方图（低延迟端糊成一桶）的病解；与 CountMinSketch/FrequencySketch（频次族）、HllCardinalitySketch（基数族）同族不同面：分位数族（spec 4039） | [spec 4039](docs/spec/4039-ddsketch-quantiles.md) |
| 事务治理 | MVCC 快照可见性 | MvccVisibility——一致性读的快照判定（PostgreSQL xmin/xmax MVCC 思想）：事务注册（begin 递增分配/commit/abort，未知与重复收尾 fail-fast）+ snapshot 捕获（xmax=下一未分配号+在飞集+已知中止集，不可变）+ 纯函数可见性判定（创建者：xmin<快照 xmax ∧ 不在飞 ∧ 未中止；删除者：缺省 ∨ ≥xmax ∨ 在飞 ∨ 已中止——删除晚于快照/未提交/回滚都对读者不存在）；repeatable read 免费成立（同快照同判定）——读写互锁（吞吐崩）与读已提交漂移（同事务重复读漂移）的病解；嵌套 Snapshot/RowVersion 不另立面；与 IdempotencyKeyGuard 互补：读侧一致性 vs 写侧去重（spec 4040） | [spec 4040](docs/spec/4040-mvcc-visibility.md) |
| 过程治理 | R 系 R42 周期对账 | Wave 7 五新类型快照补登（1158→1163：标量卡尔曼/Holt-Winters 季节指数/Theil-Sen 稳健斜率/DDSketch 分位草图——metrics×4 + MVCC 快照可见性——transaction×1）+ 全仓 16 模块 verify 三门绿（R 第七波——对账门四十二号四件套齐整）+ 42/50=84% 里程碑（spec 4041） | [spec 4041](docs/spec/4041-r-r42-reconciliation.md) |
| 文本治理 | 三方合并 | ThreeWayMerge——双端并发编辑的自动归一与冲突显形（git merge-file/diff3 思想）：以基准为锚对双方各做 LCS 行级 diff，改动按 base 区间对账——单侧改动自动取侧、双侧同改自动归一（git 同款：同改不算冲突）、双侧相接或重叠异改即冲突（保守口径：相邻也算——先后次序不可辨）；冲突块以 git 风格标记物化进结果行 + 结构化 Conflict 双读数；LCS 走格同长优先删（确定性）+ 零宽插入投影特判——后写全量覆盖（先写者编辑丢失）的病解；与 JSON Patch 同族不同面：操作重放 vs 状态对账合并（spec 4042） | [spec 4042](docs/spec/4042-three-way-merge.md) |
| 图谱治理 | Git 提交图世代号 | CommitGraph——依赖图血缘判定的精确世代号剪枝（Git commit-graph generation number 思想）：gen(v)=max(父 gen)+1（根=1）O(1) 读；isAncestor 先走世代号剪枝快道（gen(a) ≥ gen(b) 确定非祖先，不遍历）再有界 BFS（只走 gen > gen(a) 节点）精确判定；世代号精确单调无时钟偏斜——裸时间戳比对（commit-date 启发式被时钟倒挂打爆的教训）与全图遍历（大图代价爆炸）的病解；拓扑序注册 DAG-by-construction（未知父/重复 id fail-fast）；与 TopologicalSorter 同族不同面：全序排程 vs 血缘裁决（spec 4043） | [spec 4043](docs/spec/4043-commit-graph-generation.md) |
| 文本治理 | Rope 文本缓冲 | RopeBuffer——大文本编辑的树状分块缓冲（xi-editor/ropey Rope 思想）：权重平衡二叉树，叶持 ≤512 字符块、内部节点 weight=子树总长——charAt/insert/delete 按权重导航 O(log n)；insert/delete 经 split+concat 组合，超限深度触发重建（叶中位切分重排，确定性再平衡）；length/depth 读数 + 越界 fail-fast——字符串拼接/裸数组位移 O(n) 每操作（O(n²) 编辑序列）的病解（固定种子 500 操作 vs StringBuilder 圣像双等对拍钉住）；与 ThreeWayMerge 同族不同面：编辑缓冲 vs 合并对账（spec 4044） | [spec 4044](docs/spec/4044-rope-buffer.md) |
| 文本治理 | Avro 读写模式解析 | SchemaEvolution——结构化数据跨版本读写的确定性解析计划（Apache Avro Schema Resolution 思想）：字段按名（含 alias 别名表）配对，类型按加宽白名单（int→long/float/double、long→float/double、float→double，Avro widening 同表；收窄非法）；读方缺字段 → 默认值补位（usesDefault 显形）无默认 fail-fast；写方多余字段 → dropped 读数（数据被诚实丢弃可见）；Plan 结构化确定性可回放 + 不兼容 fail-fast 带字段名——无模式演化规则（写方加字段读方炸）与自由漂移（语义静默劣化）的病解；与 SessionExport 互补：导出格式跨环境 vs 字段级跨版本（spec 4045） | [spec 4045](docs/spec/4045-schema-evolution.md) |
| 策略治理 | fail2ban 封禁递升 | BanEscalation——反复滥用者的滑窗判禁与前科递升（fail2ban maxretry/findtime/bantime.increment 思想）：findTime 滑窗内失败 ≥ maxRetry 即禁（陈账按窗过期自然洗白——无窗全量计数的病解）；禁期时长 = base × multiplier^(banCount−1) 封顶 maxBanTime——惯犯成本递增（初犯与惯犯同价=违法成本递减的病解）；禁期内再失败不计数不复禁（封锁即足够）、禁即清失败窗、解禁重计；时钟注入（调用方传时，确定性可回放）+ Verdict 结构化裁决 + 定构畸形 fail-fast；与 RetryStormSentinel 互补：出向重试风暴 vs 入向滥用禁递升（spec 4046） | [spec 4046](docs/spec/4046-ban-escalation.md) |
| 过程治理 | R 系 R48 周期对账 | Wave 8 五新类型快照补登（1163→1168：三方合并/提交图世代号/Rope 文本缓冲/Avro 模式解析/fail2ban 封禁递升——policy×5 一包）+ 全仓 16 模块 verify 三门绿（R 第八波——对账门四十八号四件套齐整）+ 48/50=96% 里程碑（spec 4047） | [spec 4047](docs/spec/4047-r-r48-reconciliation.md) |
| 缓存治理 | Cache-Control 指令裁决 | CacheControlDirectives——响应缓存的标准指令解析与新鲜度裁决（RFC 9111 §5.2 思想）：解析（逗号分隔、大小写不敏感、值容忍引号、未知指令按 RFC 语义忽略但记录 unknown 读数、负 delta-seconds fail-fast）+ 优先级裁决 judge（no-store → NOT_CACHEABLE 最高、no-cache → MUST_REVALIDATE 次之、新鲜度上限 age<max-age/s-maxage → FRESH 否则 STALE；共享缓存 s-maxage 压过 max-age）；嵌套 Directives 不另立面——自造 TTL 语义（指令被无视：no-store 挡不住缓存）与裸字符串匹配的病解；与精确响应缓存（spec 53）互补：条目可用性语义归本件裁决（spec 4048） | [spec 4048](docs/spec/4048-cache-control-directives.md) |
| 过程治理 | R 系 R50 收口对账 | 快照补登 1168→1169（Cache-Control 指令裁决——policy×1）+ 全仓 16 模块 verify 三门绿（R48 协议口径）+ 五十轮全量核账封卷（spec 4000–4049 四件套零缺位、号段严格递增）+ 50/50=100% 里程碑——R 会话九波收卷（对账门×10 + 组件 41 件九族）；号段交接：S 系 5000/2151/T6101（spec 4049） | [spec 4049](docs/spec/4049-r-r50-reconciliation.md) |
| 过程治理 | S 系 S1 对账门落位 | SSession5000LedgerAuditTest——5000 系对账门（RSession4000LedgerAuditTest 公式族第六应用）：spec 5000–5049 自扩展扫描 ↔ shape/verify 票对（T6101+2(N−5000)）↔ impl（2151+N−5000）↔ README 纵深行四面互证 + 号段严格递增断言；S 系开卷（号段交接自 R 系收口声明：5000/2151/T6101）（spec 5000） | [spec 5000](docs/spec/5000-s-s1-ledger-audit.md) |
| 指标治理 | Roaring 压缩位图 | RoaringBitSet——大整数集合的容器化自适应压缩（RoaringBitmap/Lucene/Spark 思想）：高 16 位分桶（container key），桶内按元素数自适应——稠密桶（≥4096）转 long[] 位图容器、稀疏桶走 short 有序数组容器——稠密段省内存、稀疏段不浪费；add/remove/contains O(桶内 log n/O(1)) + 增量 cardinality + 桶级 and/or（操作数不变）；负值 fail-fast——裸 HashSet 全对象存储与固定粒度位图（稀疏 id 同样爆炸）的病解；与 HllCardinalitySketch 互补：精确压缩集合 vs 概率基数（spec 5001） | [spec 5001](docs/spec/5001-roaring-bitset.md) |
| 策略治理 | Fisher-Yates 无偏洗牌 | FisherYatesShuffle——顺序随机化的 O(n) 无偏原面（Fisher–Yates/Durstenfeld 变体思想）：从尾向前每步与 [0, i] 均匀取 j 交换（nextInt(i+1) 含自身——无偏的关键，取开区间即经典有偏洗牌），n! 排列等概率；原地/不可变新列表/索引排列三变体 + 种子注入确定性（同种子同排列）+ null/负 size fail-fast——排序键取模（排列分布不均）与每次全量重建（O(n²)）的病解（六排列 × 6000 种子均匀性对拍钉住）；与 DeterministicHash 同族不同面：散列位置映射 vs 排列均匀化（spec 5002） | [spec 5002](docs/spec/5002-fisher-yates-shuffle.md) |
| 缓存治理 | 有界负载一致哈希 | BoundedLoadRing——一致哈希分片的热点封顶与确定性回退（Google Consistent Hashing with Bounded Loads 思想，线性探查确定性落地口径）：稳定哈希定位起点（hash(key) mod n），满载则线性探查下一节点（确定性回退序），单节点负载 ≤ 其 capacity——哈希扎堆不雪崩；总容量耗尽 ISE 诚实拒配 + release/loadOf 读数 + removeNode 迁移确定 + 畸形 fail-fast——纯一致哈希无上限（扎堆过载）与全量重排（key 洗牌）的病解；与 RendezvousHashing（HRW 稳定选择）互补：纯稳定 vs 有界回退（spec 5003） | [spec 5003](docs/spec/5003-bounded-load-ring.md) |
| 事务治理 | Fencing Token 世代令牌护栏 | FencingTokenGuard——锁易主后旧持有者迟到写的物理拦截（Chubby fencing token/Kleppmann DDIA 界碑思想）：acquire 每次持有权取得发严格递增 token，存储侧 tryWrite 三态裁决（token=当前已发 ACCEPT、token<当前 STALE_TOKEN——旧持有者苏醒写被拦、token>当前 UNKNOWN_TOKEN——未发过的令牌诚实拒）+ 无锁 NO_LOCK；release 不重置世代（fencing 跨释放持久，旧令牌永不复用）；嵌套 Verdict 不另立面——锁服务与存储两侧状态脱节（GC 停顿/分区下旧持有者续写）的病解；与 CommitGraph 同族不同面（图谱血缘剪枝 vs 锁安全守卫）、与 SequenceFence 不同面（跨客户端锁界碑 vs 流内序栅）（spec 5004） | [spec 5004](docs/spec/5004-fencing-token.md) |
| 过程治理 | S 系 S6 周期对账 | Wave 1 四新类型快照补登（1169→1173：Roaring 压缩位图/Fisher-Yates 无偏洗牌/有界负载一致哈希/Fencing Token 护栏——metrics+policy+cache+transaction 各一）+ 全仓 16 模块 verify 三门绿（R48 协议口径）+ S 系第一波对账门（spec 5000–5004 零缺位）+ 6/50=12% 里程碑（spec 5005） | [spec 5005](docs/spec/5005-s-s6-reconciliation.md) |
| 并发治理 | Ticket Lock 票据锁 | TicketLock——互斥等待的 FIFO 公平排队（Linux 内核 ticket spinlock 思想）：lock() 原子取号（nextTicket++）并自旋等待 nowServing 追上自己的票——先到先服务（饥饿结构性排除）；unlock(ticket) 校验票号即当前号后放行（乱序 IAE fail-fast）；nowServing/nextTicket/queueLength 读数（排队深度可见）+ 自旋对虚拟线程友好（onSpinWait+定期 yield）+ 不可重入（自旋锁语义诚实入档）——裸 CAS 自旋（先到未必先得、饥饿无界）与 synchronized（无排队可见性）的病解（N 线程计数器最终精确对拍钉住）（spec 5006） | [spec 5006](docs/spec/5006-ticket-lock.md) |
| 并发治理 | Seqlock 序号锁 | SeqLock——读多写少小数据的读者零阻塞乐观读（Linux 内核 seqlock 思想）：序号奇偶标写入期（偶=稳定、奇=写入中），writeBegin/End 单写者 CAS 串行（并发写者失败退避），读者 readBegin/readEnd 乐观校验失败整读作废重试（无阻塞、无读者互斥、写者不被饿死）；sequence/isWriting 读数 + 畸形 writeEnd IAE——读写锁（读者互斥、写者饿死）与全量加锁（读路径成本高）的病解（并发二元组永不撕裂烟测钉住）；与 TicketLock 同族不同面：写者公平互斥 vs 读者乐观重试（spec 5007） | [spec 5007](docs/spec/5007-seqlock.md) |
| 并发治理 | Disruptor 环形缓冲 | DisruptorRingBuffer——高频事件排队的预分配零分配环（LMAX Disruptor 思想）：容量 2 幂预分配槽（sequence & mask 免模导航）+ claim/publish 两段式序标（publishCursor 只连续推进，乱序拒）+ 槽级序号簿（判定槽上数据属于哪个序号：offer 覆盖最旧窗口、publish 环满 ISE 消费侧过慢诚实上抛、tryConsume 过期/未发布 null 诚实缺口）——有界队列每次入/出队分配节点（GC 压力）的病解；单生产者单消费者口径（多消费者 gating 留后诚实入档）；与阻塞背压管线（spec 39）互补：有界阻塞队列 vs 预分配无锁环（spec 5008） | [spec 5008](docs/spec/5008-disruptor-ring-buffer.md) |
| 缓存治理 | Clock-Sweep 缓存驱逐 | ClockSweepCache——缓存驱逐的使用计数衰减与环形指针（PostgreSQL Buffer Manager clock-sweep 思想）：环形帧序 + 时钟指针扫描——usage=0 摘除、>0 衰减跳过（第二机会：常用页多扛几轮但不豁免）；命中 +1 封顶 MAX_USAGE（防老页计数奇高永驻）；put 覆盖不移位 + ringOrder/clockHand 确定性审计面——纯 LRU（偶发扫描洗出热页）与 LFU 全量计数（老页永驻）的病解；与精确响应缓存（spec 53）同族不同面：使用计数环形衰减 vs 访问序 TTL（spec 5009） | [spec 5009](docs/spec/5009-clock-sweep-cache.md) |
| 缓存治理 | SIEVE 缓存驱逐 | SieveCache——命中零重排的 FIFO 环 + 访问位驱逐（SIEVE 2024 论文思想，Redis 社区/高星项目已跟进）：插入序 FIFO 环 + 访问位，命中只置位不重排（lazy promotion——热路径零搬移）；驱逐指针自上次停点扫——访问位=1 清位跳过（一次保护机会）、=0 摘除（指针停在驱逐点之后）；order/hand 确定性审计面——LRU 每次命中搬移重排（热路径开销 + 扫描污染）的病解（最老已访问项跨插入存活分叉场景钉住）；与 ClockSweepCache（spec 5009）同族不同面：访问位一次清位 vs 使用计数衰减（spec 5015） | [spec 5015](docs/spec/5015-sieve-cache.md) |
| 事务治理 | Two-Phase Commit 协调器 | TwoPhaseCoordinator——跨资源原子提交的显式阶段状态机（2PC prepare 投票+裁决思想）：begin→PREPARING，votePrepare 全 yes→PREPARED（任一 no→ABORTED 一票否决），commit 仅自 PREPARED→COMMITTED、abort 自 PREPARING/PREPARED→ABORTED；非法迁移/重复投票/未知参与者/未知事务 IAE fail-fast——各资源自行提交（部分提交——原子性撕裂）与无阶段约束（ABORTED 后还能 COMMIT 静默通过）的病解；嵌套 Phase 枚举不另立面；与 FencingTokenGuard（S5）互补：锁安全守卫 vs 原子提交状态机（spec 5016） | [spec 5016](docs/spec/5016-two-phase-commit.md) |
| 过程治理 | S 系 S24 周期对账 | Wave 4 五新类型快照补登（1183→1188：Read Repair 读修复——transaction + 热点 Key 探测器/稀疏索引/区间树——metrics×3 + 有界信箱——backpressure）+ 全仓 16 模块 verify 三门绿（R48 协议口径）+ S 系第四波对账门（spec 5000–5022 零缺位）+ 24/50=48% 里程碑（spec 5023） | [spec 5023](docs/spec/5023-s-s24-reconciliation.md) |
| 过程治理 | S 系 S30 周期对账 | Wave 5 五新类型快照补登（1188→1193：MemTable 内存表/跳跃表——metrics×2 + 平滑加权轮询/亏空调度——policy×2 + 分段日志——recovery）+ 全仓 16 模块 verify 三门绿（R48 协议口径）+ S 系第五波对账门（spec 5000–5028 零缺位）+ 30/50=60% 里程碑（spec 5029） | [spec 5029](docs/spec/5029-s-s30-reconciliation.md) |
| 事务治理 | Wait-Die/Wound-Wait 死锁预防 | WoundWaitGate——并发事务抢资源的时间戳单向等待防死锁（PostgreSQL/DB2 死锁预防经典思想）：事务以开始时间戳定年龄（显式注入确定性+并列 id 字典序 tie-break），WOUND_WAIT 年长请求者枪伤年轻持有者并接管/年轻等待，WAIT_DIE 年长等待/年轻自裁——等待图单向（只等更年轻/只等更年长）环不可形成——死锁被预防而非检测；release 交接年长等待者 + holdersOf/waitersOf/abortedCount 读数 + 畸形 fail-fast——无冲突时序约定相互等待成环（检测与解除风暴）的病解；与 TwoPhaseCoordinator（S17）同族不同面：原子提交 vs 冲突时序裁决（spec 5030） | [spec 5030](docs/spec/5030-wound-wait-gate.md) |
| 恢复治理 | Leveled Compaction 分层压实 | LeveledCompaction——压实调度的层容量阶梯与区间重叠挑选（LevelDB/RocksDB leveled compaction 思想）：L0 触发线 + L_n=基准×比率^(n-1) 容量阶梯（写放大有界），plan 选最高评分层（并列浅层优先）最旧表为源、下一层键区间重叠表（firstKey 序）为目标、末层同层压实，complete 源/目标出账（产物回账在调用方）——无阶梯每层无差别扫（写放大失控）的病解；与 KeyCompaction（同键留新）同族不同面：键值保留语义 vs 层级容量挑选（spec 5031） | [spec 5031](docs/spec/5031-leveled-compaction.md) |
| 指标治理 | B+ Tree 有序索引 | BPlusTree——高扇出确定性平衡有序索引（MySQL InnoDB B+ 树思想）：键值全落叶层、内节点只放分隔键（扇出=m，树高即寻址上界），叶 next 链顺序扫描（范围扫底座）；满节点确定性对半分裂——叶分裂分隔键=左半末键复制上提（childIndexOf 严格小于计数，等键走左子树）、内分裂中位键移动上提；put upsert 覆盖不增位 + size/height 读数 + maxKeys<2 fail-fast——有序数组插入 O(n) 与平衡二叉树扇出小层多的病解（扰动 500 插入 vs TreeMap 圣像全等钉住）；与 SkipList（S26）同族不同面：概率多层链 vs 确定性多路平衡树（spec 5032） | [spec 5032](docs/spec/5032-b-plus-tree.md) |
| 存储治理 | Content-Defined Chunking 内容定义分块 | ContentDefinedChunking——字节流切块边界只由内容决定（restic/rclone FastCDC 思想）：Gear 滚动哈希（64 位旧字节自然移出无显式滑窗）低位掩码命中即切，minSize 内不切、maxSize 硬上限强切、归一化点后换更难命中的 L 掩码（切点偏大方差收窄——FastCDC 双掩码）；Gear 表 SplitMix64 种子化（跨进程同表固定切分确定性）；块覆盖连续拼接还原 + 块畸形 fail-fast——定长切块（插入移位全量失效去重率崩塌）的病解（圣像 304 块钉住+单字节翻转前稳后变钉住）；与 SegmentLog（S29）同族不同面：切块去重面 vs 追加保留面（spec 5033） | [spec 5033](docs/spec/5033-content-defined-chunk.md) |
| 策略治理 | Radix Tree 基数树路由 | RadixTree——压缩前缀树的最长前缀匹配（go-chi/httprouter 静态路由思想）：单字符边合并字符串边（公共前缀只存一份——节点数=分支点数而非字符数），insert 沿边行走部分命中即分裂出中间节点（静态建树——终态与插入序无关），match 输入尽深处最近终节点胜出；keyCount/nodeCount 压缩度读数 + null/空键/重复键 fail-fast——逐键 startsWith 线性扫（每查 O(键数×键长)）与朴素 Trie 每字符一节点（内存放大）的病解（正逆序建树 nodeCount=4 全等+roman 经典族钉住）；与 JumpHash（S31）同族不同面：前缀结构匹配 vs 无状态散列分布（spec 5034） | [spec 5034](docs/spec/5034-radix-tree.md) |
| 过程治理 | S 系 S36 周期对账 | Wave 6 五新类型快照补登（1193→1198：WoundWaitGate 死锁预防时序——transaction + 分层压实——recovery + B+ 树——metrics + CDC 内容定义分块——fs + 基数树——policy）+ S31 换静脉勘误收档（JumpHash 撞 spec 3020 换 WoundWaitGate；旧票清除）+ 全仓 16 模块 verify 三门绿（R48 协议口径）+ S 系第六波对账门（spec 5000–5034 零缺位）+ 36/50=72% 里程碑（spec 5035） | [spec 5035](docs/spec/5035-s-s36-reconciliation.md) |
| 并发治理 | EBR 时代回收 | EpochReclamation——无锁结构延迟回收的时代守卫（folly/libcds EBR 思想）：enter 守卫钉住当前全局时代（try-with-resources 即退），retire 记录退休时代，advanceEpoch 显式推进（旧守卫钉旧值直至退出——确定性无墙钟），tryReclaim 仅收「时代<全局时代且该时代及更早守卫清零」的项——不可能释放仍被读者引用的节点；按时代升序+退休序确定性回收 + currentEpoch/activeGuards/retiredCount 读数 + 双重关闭/重复退休 fail-fast——读完即 free（引用-释放竞争）与靠锁/GC（吞吐损失）的病解；与 SeqLock（S8）同族不同面：奇偶序号乐观读 vs 延迟回收生命周期（spec 5036） | [spec 5036](docs/spec/5036-epoch-reclamation.md) |
| 观测治理 | Distributed Snapshot 一致快照 | DistributedSnapshot——进程状态与在途报文切成一致瞬间的标记法（Chandy-Lamport 思想，Flink barrier/Akka 同源）：0 号发起记状态+出边发标记，进程首次收标记记自身状态+向出边续发，每信道标记到达即封口——信道记录=发起者快照后发出/标记前在途的报文（在途归信道不归进程——不丢不重），result 出进程状态+信道在途全集（未完成拒出口）；确定性事件驱动 + 重复发起/未发起收标记/已封口重复标记/空信道收报 fail-fast——各节点各自采样拼不成全局一致瞬间（鬼影状态）的病解（教科书两进程场景钉住）；与 SessionArchiver 同族不同面：本地全量导出 vs 分布式一致瞬间（spec 5037） | [spec 5037](docs/spec/5037-distributed-snapshot.md) |
| 策略治理 | Shuffle Sharding 洗牌分片 | ShuffleSharding——每租户从 n 分片随机挑 k 个的爆炸半径隔离（AWS shuffle sharding 思想）：SplitMix64(seed XOR FNV-1a 64 租户指纹) → Fisher-Yates 洗牌取前 k（升序确定性），请求只路由本租户子集（routesTo）——坏分片对单租户爆炸半径 k/n，两租户子集期望重叠 k²/n；overlap 共担面读数 + 同租户同子集 + 参数越界 fail-fast——全租户共享全部分片（一个坏分片打全部租户）的病解（圣像 4 租户钉住+200 租户平均重叠 0.0398≈0.04 钉住）；与 BoundedLoadRing（S4）同族不同面：子集隔离 vs 全局负载有界（spec 5038） | [spec 5038](docs/spec/5038-shuffle-sharding.md) |
| 缓存治理 | Segmented LRU 分段缓存 | SlruCache——试用期/保护期双段各自 LRU 的扫描防污染缓存（PostgreSQL 缓冲区管理/Caffeine SLRU 思想）：新键入试用尾、命中一次晋升保护尾、保护满降级保护头回试用尾、淘汰只走试用头——扫描键要冲垮保护段必须先被二次命中；get 晋升/containsKey 透视/upsert 覆盖晋升 + size/probationarySize/protectedSize/evictedCount 读数 + 畸形 fail-fast——普适 LRU 一次全表扫把热数据全冲走（命中率崩塌）的病解（4 扫描键后热键存活+逐段读数钉住）；与 ClockSweepCache（S10）同族不同面：使用计数衰减 vs 双段迁移（spec 5039） | [spec 5039](docs/spec/5039-slru-cache.md) |
| 指标治理 | Blocked Sliding Counter 分块滑窗计数 | BlockedSlidingCounter——窗口切 m 块每块精确计数整块进出的有界内存滑窗近似计数（Datar-Gionis 指数直方图同族分块面）：lowerEstimate=Σ块−最旧块、upperBound=Σ块——真值恒在两界之间（最旧块只知部分在窗）且界宽≤一个块计数，内存 O(m) 而非 O(N)+position/blockCount/blockSize 读数+参数 fail-fast——每事件一个计数的精确滑窗（内存随窗口线性爆）与无界衰减估计（窗口界不可知）的病解（500 位扫描真值逐位落界钉住）；与 SlidingWindowCounter（限流）同族不同面：单位桶限流计数 vs 近似计数+显式误差界（spec 5040） | [spec 5040](docs/spec/5040-blocked-sliding-counter.md) |
| 过程治理 | S 系 S42 周期对账 | Wave 7 五新类型快照补登（1198→1203：EBR 时代回收——concurrent + 一致快照——observability + 洗牌分片——policy + 分段 LRU——cache + 分块滑窗计数——metrics）+ 全仓 16 模块 verify 三门绿（R48 协议口径）+ S 系第七波对账门（spec 5000–5040 零缺位）+ 42/50=84% 里程碑（spec 5041） | [spec 5041](docs/spec/5041-s-s42-reconciliation.md) |
| 指标治理 | Xor Filter 异或过滤器 | XorFilter——静态键集的紧凑指纹成员判定（Graf-Lemire xor filter 思想，ClickHouse/DuckDB 社区 Bloom 后继）：三独立散列定位三槽+8 位指纹，查询=三槽异或等于键指纹（单次数组访问级）——成员恒真无假阴性、假阳性 ~2^-8 量级、~1.23×键数槽位比 Bloom 更省；剥洋葱构建（度一边摘除逆序回填）失败换盐重试（确定性盐序列）+checksum 同键集同校验和审计读数+null/空/重复 fail-fast——Bloom 位阵（10+ bits/键 k 次探针）与精确集合（内存线性爆）的病解；与 SessionBloomFilter/CuckooFilter 同族不同面：静态异或集 vs 动态位阵/布谷鸟指纹（spec 5042） | [spec 5042](docs/spec/5042-xor-filter.md) |
| 事务治理 | CRDT PN-Counter 正负计数器 | CrdtPnCounter——无协调可合并的分布式计数（Riak/Redis CRDT 正负计数器思想）：P（增）N（减）两个 G-Counter 每节点单调计数表，本地增减、merge 按节点取 max——交换/幂等/结合三律成立任意顺序同步必收敛，value=P−N 可负；incrementEntries/decrementEntries 不可变审计读数+null/空节点/负量 fail-fast——中心序列化（协调开销+可用性损失）与 last-write-wins 覆盖（并发增量互吞）的病解；与 ReadRepair（S19）互补：读路径修复 vs 可合并状态（spec 5043） | [spec 5043](docs/spec/5043-crdt-pn-counter.md) |
| 指标治理 | Segment Tree 线段树 | SegmentTree——区间和与点更新同取对数的序列聚合（线段树经典 ZKW 2n 迭代式实现思想）：叶层存原值、父节点存子区间和（无递归无 4n 空间），rangeSum O(log n)、update 自叶向上回填 O(log n)+size 读数+区间越界/倒置/下标越界 fail-fast——区间和线性扫（查询 O(n) 放大）与前缀和数组（点更新 O(n) 重算）两种病的同解（136 子区间暴力圣像全等钉住）；与 FenwickTree 同族不同面：前缀和 BIT vs 任意区间和+点更新；与 IntervalTree（S23）不同面：stabbing 区间集合 vs 序列聚合（spec 5044） | [spec 5044](docs/spec/5044-segment-tree.md) |
| 指标治理 | Robin Hood Hash Table 劫富济贫哈希表 | RobinHoodHashTable——线性探测插入时来键距离超在位键即换位的均衡哈希表（Robin Hood 开放寻址思想）：富者让位贫者被换键携距离续探——距离方差收窄长簇截断，删除后向搬移双索引回填无墓碑，0.75 负载倍容+size/capacity/resizeCount/maxProbeDistance 均衡读数+畸形 fail-fast——线性探测聚类越长越长（命中距离无上界）与墓碑堆积（探测链腐化）的病解（跨环绕簇换位 maxProbeDistance=3 钉住）；与 MemTable（S25）同族不同面：哈希无序 O(1) vs 有序 O(log n)（spec 5045） | [spec 5045](docs/spec/5045-robin-hood-hash.md) |
| 缓存治理 | TinyLFU Admission 准入策略 | TinyLfuAdmission——候选频次≥受害者才准入的裁决面（Caffeine W-TinyLFU 准入思想）：4 位饱和计数 sketch 两行 count-min（估计取行最小抑制碰撞高估），record 饱和加、admit 候选估计≥受害者估计——热者不让位冷者不挤占，记录达阈值全体减半老化（旧热点平稳淡出 sketch 不被历史占死）+estimate/resetCount/sinceReset/resetEvery 读数+null/参数 fail-fast——新键无条件挤掉受害者（一次性扫描冲垮缓存）与频率表被历史占死的病解（大容量饱和 15+第 8 记自增后减半 8→4 钉住）；与 SlruCache（S40）同族不同面：准入裁决 vs 双段驱逐（spec 5046） | [spec 5046](docs/spec/5046-tiny-lfu-admission.md) |
| 过程治理 | S 系 S48 周期对账 | Wave 8 五新类型快照补登（1203→1208：Xor Filter 异或过滤器/线段树/劫富济贫哈希表——metrics×3 + CRDT PN-Counter——transaction + TinyLFU 准入——cache）+ 全仓 16 模块 verify 三门绿（R48 协议口径）+ S 系第八波对账门（spec 5000–5046 零缺位）+ 48/50=96% 里程碑（spec 5047） | [spec 5047](docs/spec/5047-s-s48-reconciliation.md) |
| 并发治理 | Pairing Heap 配对堆 | PairingHeap——meld O(1) 挂钩的可合并最小堆（Fredman-Sedgewick 配对堆思想，Haskell containers/Boost 同源）：多叉树最小堆，insert=单节点 meld、meld=根比较挂钩（所有权语义他堆清空）、extractMin 后孩子两趟合并（先左到右两两配对再右到左并回，摊还 O(log n)）+size/isEmpty 读数+空堆/self-meld fail-fast——二叉堆 meld O(n) 复制（可合并性缺失）与二项堆复杂不变量的病解（300 键 TreeMap 多重集圣像+同操作同出序钉住）；与 AgingPriorityQueue（exec）同族不同面：老化策略 vs 可合并结构（spec 5048） | [spec 5048](docs/spec/5048-pairing-heap.md) |
| 过程治理 | S 系 S50 收口对账 | Pairing Heap 配对堆快照补登（1208→1209：可合并堆——concurrent）+ 全仓 16 模块 verify 三门绿（R48 协议口径）+ S 系终验对账门（spec 5000–5049 全 50 轮四件套零缺位）+ 50/50=100% 里程碑——S 会话全闭合（spec 5049） | [spec 5049](docs/spec/5049-s-s50-reconciliation.md) |
| 过程治理 | T 系 T1 对账门落位 | TSession6000LedgerAuditTest——6000 系对账门（SSession5000LedgerAuditTest 公式族第七应用）：spec 6000–6049 自扩展扫描 ↔ shape/verify 票对（T6201+2(N−6000)）↔ impl（2201+N−6000）↔ README 纵深行四面互证 + 号段严格递增断言；T 系开卷（号段交接自 S 系收口声明：6000/2201/T6201）（spec 6000） | [spec 6000](docs/spec/6000-t-t1-ledger-audit.md) |
| 并发治理 | Splay Tree 伸展树 | SplayTree——访问即伸展的自调整有序映射（Sleator-Tarjan 伸展树思想，GCC splay-tree 同源）：put/get/remove 后目标键三式旋转（zig/zig-zag/zig-zig）自顶向下上浮至根——热键恒近根摊还 O(log n)，节点零平衡元数据；put upsert 覆盖不增位、get 缺席 null、remove 嫌席 fail-fast + size/rootKey 结构确定性读数 + keysInOrder 中序全序——普通 BST 顺序插入退化链（操作 O(n)）与平衡树旋转不变量繁多（实现复杂）的病解（500 键顺序插入中序全等+扰动 TreeMap 圣像+访问后根=被访键+删除后根=前驱钉住）；与 PairingHeap（S49）同族不同面：优先序堆 vs 全序映射（spec 6001） | [spec 6001](docs/spec/6001-splay-tree.md) |
| 并发治理 | Treap 树堆 | Treap——BST 键序+最小堆优先级双不变量的期望平衡有序映射（Seidel-Aragon 树堆思想）：新节点优先级种子化 SplitMix64（同种子同结构确定性可回放），插入沿路径单旋上浮、remove 旋降至叶摘除——期望 O(log n) 平衡只靠「优先级随机+单旋转」拿到，AVL/红黑旋转不变量全免+size/height 平衡度读数+keysInOrder 中序全序+upsert 覆盖不增位不换优先级+缺席 null/嫌席 fail-fast——普通 BST 顺序插入退化链（O(n)）与平衡树实现复杂（不变量繁多）的病解（500 键顺序插入 height≤32+TreeMap 圣像+同种子双实例同构/异种子分异钉住）；与 SplayTree（T2）同族不同面：访问自调整 vs 先验随机平衡（spec 6002） | [spec 6002](docs/spec/6002-treap.md) |
| 指标治理 | Sparse Table 稀疏表 | SparseTable——倍增表预计算+O(1) 幂等重叠查询的静态序列最小值面（Bender-Farach 静态 RMQ 思想）：sparse[k][i]=从 i 起长 2^k 的最小值（O(n log n) 一次性预计算不可变），rangeMin 取覆盖区间的两个 2^k 块 min（幂等聚合允许重叠——k=⌊log₂(len)⌋ 两次访存出值）+size 读数+null/空/倒置/越界 fail-fast——每查一遍线性扫（重复查询 O(n) 放大）与为不发生的更新支付对数代价（线段树面）的病解（17 元素 136 子区间暴力圣像+1000 元素 1000 随机查询全等钉住）；与 SegmentTree（S45）同族不同面：静态 O(1) 幂等重叠 vs 动态点更新 O(log n)（spec 6003） | [spec 6003](docs/spec/6003-sparse-table.md) |
| 过程治理 | T 系 T6 周期对账 | Wave 1 四新类型快照补登（1209→1213：SplayTree/Treap——concurrent + SparseTable/MonotonicDeque——metrics）+ 全仓 16 模块 verify 三门绿（R48 协议口径）+ T 系第一波对账门（spec 6000–6003 零缺位）+ 12% 里程碑（spec 6005） | [spec 6005](docs/spec/6005-t-t6-reconciliation.md) |
| 指标治理 | Monotonic Deque 单调队列 | MonotonicDeque——定容窗严格递减队列的流式摊还 O(1) 最大值面（单调队列滑窗最值思想）：offer 队尾 ≤ 新值者从尾弹出（弱者没有未来——出窗更早且值更小）、队首越窗从首弹出——队首恒为窗最大，每元素至多入队/出队各一次（摊还 O(1) 无墓碑）+max/size 读数+capacity≤0 fail-fast——每窗重扫 O(k·n)（滑动放大）与有序堆惰性墓碑删除（O(n log k) 删除语义腐化）的病解（500 点窗 7 暴力扫圣像全程全等+升/降/全等输入+越窗弹出逐值钉住）；与 SparseTable（T4）同族不同面：静态离线 O(1) vs 流式在线摊还 O(1)（spec 6004） | [spec 6004](docs/spec/6004-monotonic-deque.md) |
| 文本治理 | Aho-Corasick 自动机 | AhoCorasick——Trie+BFS 失配链的文本单次扫描多模式全命中（Aho-Corasick 多模式匹配思想，ripgrep/安全扫描器同源）：当前节点失配沿 fail 回退复用后缀状态——O(文本长+命中数) 与模式数无关，节点持模式索引列表（重复注册不折叠）；Match(pattern,start,end) canonical 序（起始升序+模式文本字典序，重复模式并列）+patternCount 读数+null/空模式与 null 文本 fail-fast——每模式独立 indexOf 扫（O(模式数×文本长) 放大失控）的病解（ushers 经典三命中+aaaa 重叠三命中+30 模式×400 文本暴力圣像+中文模式集钉住）；与 KmpSearch（Q 系）同族不同面：单模式前缀函数 vs 多模式自动机一次扫（spec 6006） | [spec 6006](docs/spec/6006-aho-corasick.md) |
| 文本治理 | Myers O(ND) Diff | MyersDiff——贪心 O(ND) 最短可执行编辑脚本（Myers 差分论文思想，git diff 同源）：外层步数 d、对角线 k 的 V 数组推进+每步轨迹快照回溯产出三态脚本（EQUAL/INSERT/DELETE），脚本长度 = N+M−2·LCS 最优；tie-break 定构（k=−d 先降后删——同输入同脚本可回放）+静态工具面+null 序列 fail-fast——全量替换（差异无结构不可审）与朴素全量 DP O(N·M) 时空（大序列不可行）的病解（脚本应用于 a 恒得 b+EQUAL 双侧一致+200 对随机小序列 DP oracle 最优性钉住；双空序列 v 数组越界由早退守卫根治）；与 ThreeWayMerge（spec 4042）同族不同面：双侧 diff 底座 vs 三方归一（spec 6007） | [spec 6007](docs/spec/6007-myers-diff.md) |
| 文本治理 | BK 树 | BkTree——距离分叉+三角不等式剪枝的编辑距离邻域索引（Burkhard-Keller 度量树思想，拼写检查/模糊搜索同源）：节点按「到父词编辑距离」分叉，查询对节点算 d(node,word) 后只下探距离 ∈ [d−r, d+r] 的分支（三角不等式保证剪枝不漏）；动态 add 词集增量友好、d=0 重复词幂等不增位、query 结果字典序 canonical 输出+size 读数+null 词/r<0 fail-fast——每查询全字典 O(N·L²) 扫（词库大时放大失控）的病解（book/back/buck/bock 经典例+150 词库×60 查询×r∈{0,1,2} 逐词暴力圣像全等钉住）；与 TextDistance（2052）同族不同面：距离度量 vs 邻域索引结构（spec 6008） | [spec 6008](docs/spec/6008-bk-tree.md) |
| 文本治理 | Suffix Array 后缀数组 | SuffixArray——倍增构造+Kasai LCP 的静态文本一次建索引多查询面（Manber-Myer 思想，Lucene/生信索引同源）：rank 对排序倍增 O(n log²n) 建后缀序、Kasai O(n) 求 LCP，contains/occurrenceCount 二分后缀序（O(m log n) 重叠出现全计——初版 contains 以「≥」代「=」由 banana 例钉住修正等值判定）+suffixArray/lcpArray 防御性副本读数（结构可独立复算审计）+null/空文本与 null/空查询 fail-fast——每查询对全部后缀 startsWith 线性扫（O(n·m) 每查放大）的病解（banana 经典 sa/lcp 全序+300 随机文本 100 查询暴力全等+后缀序两两有序/LCP 直算钉住）；与 AhoCorasick（T7）同族不同面：模式集索引文本 vs 文本索引查询集（spec 6009） | [spec 6009](docs/spec/6009-suffix-array.md) |
| 溢出治理 | Piece Table 文本缓冲 | PieceTable——原稿只读+追加型增量片表的编辑不复制缓冲（VSCode/Word 文本缓冲思想）：片段表按序引用原稿/增量缓冲区间拼装视图，insert/delete 只增删/切分片段不搬原文——初版 delete 两处真缺陷（裁剪后 cur 未按保留长推进致后续片段坐标漂移；split 右片段起点漏加 cutStart 致被删字符复活）由 500 步随机 StringBuilder oracle 钉住根治，并确立「删除区间随删随缩」remaining 口径 +text/length/pieceCount/addedLength 增量可见读数+越界/畸形 fail-fast——每编辑复制全文（O(n) 编辑放大，大文档卡顿）的病解（500 混合编辑 oracle 全等+尾插/中点切分/跨片段删除逐值钉住）；与 RopeBuffer（4044）同族不同面：权重树拼接 vs 原稿+片表双缓冲（spec 6010） | [spec 6010](docs/spec/6010-piece-table.md) |
| 过程治理 | T 系 T12 周期对账 | Wave 2 五新类型快照补登（1213→1218：AhoCorasick/MyersDiff/BkTree/SuffixArray——metrics + PieceTable——fs）+ 全仓 16 模块 verify 三门绿（R48 协议口径）+ T 系第二波对账门（spec 6000–6010 零缺位）+ 24% 里程碑（spec 6011） | [spec 6011](docs/spec/6011-t-t12-reconciliation.md) |
| 消息治理 | Elias-Fano 单调序列编码 | EliasFano——高低位联合位图的近下界压缩+O(1) 随机访问（Elias-Fano 编码思想，Lucene/倒排索引同源）：lowerWidth=⌈log₂⌈U/n⌉⌉ 拆高低位——低位定宽直存、高位「值+下标」联合位图（每值一置位——间隔即增量），get O(1)（位图 select+低位切片）+decode 全序还原+size/lowerWidth/storedBits 压缩率读数+null/空/非单调/越界 fail-fast——long 数组直存 8 字节/元素（内存放大）与差分链表随机访问 O(n)（读放大）两种病的同解（八组密度用例往返全等+低位宽公式逐例 1/20/6/0 钉住+密集 storedBits<32×n；初版整数除法算宽判错改 ceilDiv+严格 ceil-log2）；与 DeltaFrameOfReference 同族不同面：分块差分参照 vs 高低位联合位图（spec 6012） | [spec 6012](docs/spec/6012-elias-fano.md) |
| 消息治理 | Gorilla XOR 浮点压缩 | GorillaXor——相邻 double XOR 三态控制位流的时序浮点无损压缩（Facebook Gorilla/Prometheus TSDB 思想）：同值 1 位 0；有效位落前值窗内复用（'10'+有效位）；否则新窗（'11'+5 位前导钳 31+6 位有效长−1+有效位）；decompress 按位无损（rawBits——NaN 载荷/±0/Inf 保真）+compressedBits/originalBits/count 压缩率对账读数+null fail-fast——时序浮点逐值 8 字节直存（本地性序列信息冗余极高）的病解（量化步进 500 点无损+压缩<1/3+随机 200 点无损+特殊值按位保留+全同序列恒 64+99 位钉住）；与 VarintCodec/DeltaFrameOfReference 同族不同面：整数变长/差分参照 vs 浮点 XOR 有效位窗（spec 6013） | [spec 6013](docs/spec/6013-gorilla-xor.md) |
| 消息治理 | Simple8b 位打包 | Simple8b——4 位选择子+60 位载荷 16 档变长打包的小整数列共居面（Simple8b 思想，InfluxDB/时序列存同源）：档位「值数×位宽」从 240×0 到 1×60 贪心选首个可容档（同输入同字流确定性），全零档单字 240 值、decode 尾部截断到原值数+wordCount/valueCount/wordsCopy 审计读数+null/负值/值≥2^60 fail-fast——小基数整数列逐值 8 字节直存（存储放大数十倍）的病解（混合量级 500 值往返全等+960 零恰 4 字+尾零 1 字截断+60 位值满字选择子 15 钉住）；与 VarintCodec 同族不同面：流式逐字节 vs 字对齐并行；与 BitPacking（T16）互补：混合自适应 vs 单一固定宽（spec 6014） | [spec 6014](docs/spec/6014-simple8b.md) |
| 消息治理 | Bit Packing 固定位宽打包 | BitPacking——统一位宽无缝串接的列存定宽打包面（Parquet/ORC bit-packing 思想）：每值占定宽 w∈[0,64] 位首尾相接跨字排布（64 位字界零浪费），跨字双字移位合并 O(1) 取值+unpack 全序还原+bitWidth/count/wordCount（⌈n·w/64⌉）/packedCopy 审计读数+null/w 越域/负值/溢出 fail-fast——全量 long 直存（值域窄时放大数十倍）的病解（九档位宽 {0,1,3,7,31,32,33,63,64} 往返全等+宽 7×100 值 11 字跨字逐值钉住；初版 63 位域 limit=1<<63 溢出负数误判拆分校验根治）；与 Simple8b（T15）互补：单一固定宽 vs 混合自适应档位（spec 6015） | [spec 6015](docs/spec/6015-bit-packing.md) |
| 消息治理 | Dictionary Encoding 字典编码 | DictionaryEncoding——低基数列拆「字典表+变窄下标列」的双层编码（Parquet/ORC 字典编码思想）：首次出现序登记字典（值→下标），下标列以 ⌈log₂(去重数)⌉ 位定宽打包（组合 BitPacking 层间解耦各自可审计；单值列 0 位宽）+decode/valueAt 还原+dictionary/distinctCount/indexBitWidth/size/indicesWordCount 压缩对账读数+null/空列/越界 fail-fast——重复率高的列（枚举/状态/标签）逐值全宽直存（存储放大）的病解（5 基数 300 行往返+字典首次出现序+位宽 3 钉住+1000 行双值 16 字对账）；与 BitPacking（T16）组合不同层：下标列底座；与 Simple8b（T15）不同面：基数收缩 vs 同域变长（spec 6016） | [spec 6016](docs/spec/6016-dictionary-encoding.md) |
| 过程治理 | T 系 T18 周期对账 | Wave 3+4 十新类型快照补登（1218→1228：Wave3×5 EliasFano/GorillaXor/Simple8b/BitPacking/DictionaryEncoding——message + Wave4 预载×5 KdTree/QuadTree/Geohash/HilbertCurve——policy + InterpolationSearch——concurrent）+ 全仓 16 模块 verify 三门绿（R48 协议口径）+ T 系第三波对账门（spec 6000–6016 零缺位）+ 36% 里程碑（spec 6017） | [spec 6017](docs/spec/6017-t-t18-reconciliation.md) |
| 策略治理 | KD-Tree 二维最近邻 | KdTree——交替轴中位数分割+回溯剪枝的静态二维最近邻（kd-tree 思想，scikit-learn/图形学同源）：建树每层按 x/y 轴交替取中位数分割，查询沿目标侧下探、回溯时到轴平面距离平方≤当前最优才探另一侧；long 坐标距离平方无浮点误差+平局 canonical（距离同→x 小→y 小——同查询同结果）+null/空/行非二维 fail-fast——每查询全点集线性扫（O(n) 放大）的病解（200 点×100 查询暴力圣像含平局规则双侧一致+共线退化+重复点平局+负坐标钉住；初版深层最优丢失与二次间接两缺陷由该圣像钉住改 [bestIdx,bestDist] 对返回）；与 QuadTree（T20）同族不同面：轴分割最近邻 vs 象限桶区域查询（spec 6018） | [spec 6018](docs/spec/6018-kd-tree.md) |
| 策略治理 | QuadTree 四叉树 | QuadTree——桶容量四分+矩形相交剪枝的点域四叉树（地理围栏/空间索引四叉树思想）：桶容量 4 超限四分（子界 [midX+1,maxX] 与象限判定 >midX 严格一致——边界点不漂移），区域查询按「矩形与节点界相交」剪枝+结果 (x,y) 字典序 canonical+退化单格根自动停分+根界倒置/越界插入/查询倒置 fail-fast——每查询全点集线性过滤（O(n) 放大）的病解（300 点×30 随机矩形暴力圣像含字典序+象限边界点 7/8 线一致性+退化单格钉住；源码随 T18 对账批预入档）；与 KdTree（T19）同族不同面：象限桶区域查询 vs 轴分割最近邻（spec 6019） | [spec 6019](docs/spec/6019-quad-tree.md) |
| 策略治理 | Geohash 地理哈希 | Geohash——经纬交替二分 5 位 Base32 的空间前缀编码（Redis GEO/位置服务 geohash 思想）：经度偶数位起交织，前缀共享即空间邻近（同格/邻格共享长前缀），decode 还原包围盒+中心（encode/decode 互逆定构）+lat/lon 域外/精度越域/非法字符 fail-fast——两点阈值邻近逐对计算（无法用索引前缀剪枝）的病解（Wikipedia 经典锚 ezs42+200 随机点互逆+包围盒随精度收缩+前缀共享邻近性钉住；初版锚值 7zzz 系记忆错误，边界归上半手推复核改 s 锚）；与 ZOrderCurve 同族不同面：经纬网格串 vs 平面填充序（spec 6020） | [spec 6020](docs/spec/6020-geohash.md) |
| 策略治理 | Hilbert Curve 希尔伯特曲线 | HilbertCurve——二维网格↔一维索引双射+局部性保持的空间填充曲线（PostGIS/地图瓦片思想）：标准 skew 递归（无查表无浮点），相邻格子索引差有界（Z 序象限跳变被规避），index/coordinate 互逆+order≤31（long 域）+阶/坐标/索引越界 fail-fast——行序/字典序破坏空间局部性（邻点散落全域，缓存与 IO 放大）的病解（order4 全 256 格双射穷举+端点锚（终点 (side−1,0) 变体锚）+order6 单位步平均索引差<全域/4 钉住；源码随 T18 对账批预入档）；与 ZOrderCurve 同族不同面：希尔伯特连续走位 vs Morton 交织跳变（spec 6021） | [spec 6021](docs/spec/6021-hilbert-curve.md) |
| 并发治理 | Interpolation Search 插值查找 | InterpolationSearch——值域线性内插估位的分布感知查找（数据库统计/数值库插值探测思想）：pos=lo+(key−a[lo])·(hi−lo)/(a[hi]−a[lo])，double 先行减法防 long 溢出+[lo,hi] 钳制保证正确性不依赖分布（最坏退化 O(n) 有序扫诚实边界）+等值窗口零除守卫+重复值任一位次/缺席 -1+null fail-fast——均匀有序数组对半二分无视分布信息（均匀域仍 O(log n) 探测）的病解（均匀 1000 数组全键命中+50 数组×50 查询 Arrays.binarySearch 圣像+MIN/0/MAX 极值钉住；源码随 T18 对账批预入档）；与 Arrays.binarySearch 同族不同面：分布感知内插 vs 固定对半（spec 6022） | [spec 6022](docs/spec/6022-interpolation-search.md) |
| 过程治理 | T 系 T24 周期对账 | Wave 4 文档落档核账封波（快照无需变更——1228 已含 Wave 4×5，T18 批补登）+ 全仓 16 模块 verify 三门绿（R48 协议口径）+ T 系第四波对账门（spec 6000–6022 零缺位）+ 48% 里程碑（spec 6023） | [spec 6023](docs/spec/6023-t-t24-reconciliation.md) |
| 并发治理 | MPSC 有界队列 | MpscQueue——预分配环+双单调序号的多生产者单消费者有界 FIFO（JCTools MpscArrayQueue 思想）：生产端互斥占位写入（同监视器保可见性）、消费端单线程无锁推进——两侧序号各自单调，满拒新 offer false 诚实背压、poll 空返回 null+size/capacity/isEmpty 读数+capacity≤0/null 元素 fail-fast——无界队列积压不可见（OOM 隐患）与双锁队列消费端争用（吞吐损失）两种病的同解（4 生产者×2000 单消费不丢不重+单产内保序+容量竞争上界钉住）；与 DisruptorRingBuffer（5008）同族不同面：多生产者 FIFO vs 单生产者事件派环；与 BoundedMailbox（5020）不同面：并发生产端 vs 策略性溢出（spec 6024） | [spec 6024](docs/spec/6024-mpsc-queue.md) |
| 并发治理 | Striped Lock 条带锁 | StripedLock——SplitMix64 混淆+位掩码取模的固定条带锁池（Guava Striped 思想）：n 把 ReentrantLock（向上取 2 的幂）覆盖无限键空间——相近键散开、热键恒同锁，withLock(key, action) 两条重载+stripeCount/indexFor 审计读数+stripes≤0 fail-fast——每键一把锁（内存随键基数爆炸）与全局单锁（无关键互相阻塞）两种病的同解（同键恒同锁实例+4 线程×5000 同键互斥计数精确+不同键并行重叠观测钉住；源码随 T24 核账批预入档）；与 TicketLock（5006）同族不同面：单锁 FIFO 公平 vs 键空间分条降争用（spec 6025） | [spec 6025](docs/spec/6025-striped-lock.md) |
| 并发治理 | Indexed Heap 索引堆 | IndexedHeap——id→堆位哈希映射的 O(1) 定位+decrease-key 二叉最小堆（Dijkstra/Prim 索引堆思想）：updatePriority 双向（降级上浮/升级下沉各走对数），同一 id 唯一在堆，堆序确定性（优先级同按 id 字典序——同操作序列同出序）+peekMinPriority/priorityOf/contains 读数+重复 push/缺席更新/空堆取弹 fail-fast——朴素堆更新需全扫 O(n) 的病解（300 随机出序优先级不降+decrease-key 前插+increase 后沉+混合序列终态逐值钉住；源码随 T24 核账批预入档）；与 PairingHeap（5048）同族不同面：decrease-key 一等公民 vs 可合并堆（spec 6026） | [spec 6026](docs/spec/6026-indexed-heap.md) |
| 并发治理 | Stride Scheduler 步幅调度 | StrideScheduler——pass 单调记账的确定性比例份额调度（MIT exokernel stride 思想）：客户端按票数取步幅 stride=BASE/tickets，每被调度一次 pass+=stride，serve 恒选 pass 最小者（并列 id 小——完全确定性无随机），长程 served 次数精确∝票数+remove/passOf/ticketsOf 读数+票数≤0/重复注册/缺席 fail-fast——WRR 短窗倾斜与彩票调度随机的病解（{1,1,2} 400 次调度恰 {100,100,200}+两客户端严格交替+同注册双实例序列全等钉住；源码随 T24 核账批预入档）；与 WeightedRoundRobin（5026）同族不同面：平滑插值轮询 vs 单调 pass 记账（spec 6027） | [spec 6027](docs/spec/6027-stride-scheduler.md) |
| 并发治理 | MLFQ 多级反馈队列 | Mlfq——多级队列+量子随级翻倍+用满降级沉底的行为反馈调度（OS 调度经典 MLFQ 思想）：新任务进最高级（交互优先），量子用满未完成降一级，同级 FIFO，协作式单运行槽（dispatch→onQuantumExpired/onCompleted 回报告）+activeTasks/readyAt 逐级读数+级数/量子/重复提交/错误回报告 fail-fast——交互任务被 CPU 密集任务压死与固定优先级饿死低级两种病的同解（长任务逐级沉底+量子 2→4→8 翻倍+底级不降+新任务插队最高级钉住；源码随 T24 核账批预入档）；与 EdfScheduler 同族不同面：截止时间优先 vs 行为反馈降级（spec 6028） | [spec 6028](docs/spec/6028-mlfq.md) |
| 过程治理 | T 系 T30 周期对账 | Wave 5 五新类型快照补登（1228→1233：MpscQueue/StripedLock/IndexedHeap/StrideScheduler/Mlfq——concurrent×5）+ 全仓 16 模块 verify 三门绿（R48 协议口径）+ T 系第五波对账门（spec 6000–6028 零缺位）+ 60% 里程碑（spec 6029） | [spec 6029](docs/spec/6029-t-t30-reconciliation.md) |
| 记忆治理 | Buddy Allocator 伙伴分配器 | BuddyAllocator——2 的幂块分裂/伙伴逐级合并的单位分配器（Linux 内存管理 buddy 思想）：申请 order k 无块时把更大块对半裂开（低半自用高半入闲链），释放与伙伴（offset XOR 2^k）逐级合并——外部碎片收敛回整块；freeList 用 TreeSet（同阶取最小偏移——确定性可回放）+freeBlocks 逐阶读数+阶越域/双重释放/池满 fail-fast——空闲总量够但连续块不足的病解（16 块全分配+全释放回归单 order4 块合并完备性+分裂取低半确定性+交错释放读数「伙伴未整块不并」钉住）；与 SlabClassPacker（cache）同族不同面：分裂合并 vs 尺寸分类装箱（spec 6030） | [spec 6030](docs/spec/6030-buddy-allocator.md) |
| 溢出治理 | External Merge Sort 外归并排序 | ExternalMergeSort——内存窗切块排序成游程+多路归并的外部排序（Spark/数据库外部排序思想）：⌈n/chunkSize⌉ 游程各自窗内排序，归并逐位取最小（并列游程序靠前——确定性），runCount=⌈n/chunk⌉ 显形 IO 轮次+sorted/runCount/chunkSize/size 读数+null 数据/窗≤0 fail-fast——数据远超内存时全内存排序 OOM 的病解（20 组随机 250 值窗 64 Arrays.sort 圣像+游程公式 4/3/1/0 边界+单块重复值钉住；源码随 T30 对账批预入档）；与 BuddyAllocator（T31）同族不同面：分配器 vs 排序管道（spec 6031） | [spec 6031](docs/spec/6031-external-merge-sort.md) |
| 指标治理 | Extendible Hashing 可扩目录哈希 | ExtendibleHashing——目录深度按需翻倍+局部桶分裂的哈希表（Fagin 可扩目录哈希思想）：键按哈希低 globalDepth 位寻址目录项，桶容量 4 溢出时 localDepth==globalDepth 先翻倍目录（其余项平移复制）再按 localDepth+1 重散布分裂该桶——扩容只动单桶无全量重哈希停顿；集合语义重复幂等+SplitMix64 混淆确定性分布+size/directorySize/globalDepth/bucketCount 读数——静态哈希表满即全量重哈希（扩容停顿放大）的病解（300 键全命中+缺席+重复幂等+深度单调翻倍触发+初始形态钉住）；与 RobinHoodHashTable（5045）同族不同面：开放寻址均衡探测 vs 目录翻倍桶分裂（spec 6032） | [spec 6032](docs/spec/6032-extendible-hashing.md) |
| 策略治理 | Elevator Scan 电梯扫掠 | ElevatorScan——显式磁头+方向 LOOK 扫掠的服务排序（磁盘调度 SCAN/LOOK 思想）：沿当前方向服务至该向最远请求再折返（不到物理端），同向距离升序，服务后磁头停末次轨道、方向翻转（状态可审计）；TreeSet 重复请求幂等+pendingCount/head/direction 读数+磁头/轨道越域/方向 null fail-fast——FIFO 调度往返空跑（寻道放大）与 SSTF 近者插队饿死远端的病解（经典教科书请求集上行序 82,140,170,190,43,24,16 钉住+下行对称+磁头自身请求最先；源码随 T30 对账批预入档）；与 DeficitRoundRobin（5027）同族不同面：寻道路径扫掠 vs 公平份额记账（spec 6033） | [spec 6033](docs/spec/6033-elevator-scan.md) |
| 记忆治理 | Arena Allocator 竞技场分配器 | ArenaAllocator——线性 bump 水位指针 O(1) 分配+整池回收的竞技场（Netty/Flink pooled arena 思想）：分配只前移水位（无逐块元数据），freeAll 一次归零，highWaterMark 历史峰值显形（审计——不被回收清零）；单块 free 显式抛出（竞技场语义诚实边界）+watermark/highWaterMark/capacity 读数+capacity≤0/size≤0/越池/越界 fail-fast——小对象高频分配走通用堆（GC 压力放大）的病解（顺序偏移 0/10/30 确定性+freeAll 归零峰值保留+回收后读零钉住；源码随 T30 对账批预入档）；与 BuddyAllocator（T31）同族不同面：线性 bump+整池回收 vs 2 的幂分裂合并（spec 6034） | [spec 6034](docs/spec/6034-arena-allocator.md) |
| 指标治理 | Counting Bloom Filter 计数布隆过滤器 | CountingBloomFilter——k 哈希计数数组可删除布隆（Bloom 1970 计数变体思想）：插入 +1、删除 −1 饱和于 0，contains 判定 k 位计数全 >0——无假阴性可删除，假阳性率 ≈(1−e^{−kn/m})^k 经验受控；SplitMix64 双哈希 h1+i·h2 模拟 k 探针（确定性）+insertedCount/counterCount/numHashes 读数+预期插入/假阳性率/哈希数/null 越域 fail-fast——标准布隆位阵不可删除（重建才可移除）的病解（500 插入零假阴性+删除即缺席+重复计数守恒+2000 探针假阳性 ≤5% 钉住）；与 SessionBloomFilter/XorFilter（5042）同族不同面：可删除动态计数 vs 位阵/静态指纹（spec 6036） | [spec 6036](docs/spec/6036-counting-bloom.md) |
| 指标治理 | Stable Bloom Filter 稳定布隆过滤器 | StableBloomFilter——确定性游标衰减位阵的流式「最近见过」成员判定（Stable Bloom 思想）：每次插入前将 d 个游标位衰减 −1，再按 k 哈希置满（饱和 3），查询判 k 位全非零——旧元素停插即淡出（标准/计数布隆一旦插入永久占据）；SplitMix64 游标+双哈希探针（同流同行为确定性）+cellCount/decayCount 读数+槽位/衰减/哈希数越域与 null fail-fast——「最近见过」语义无法表达的病解（1000 近期插入恒真+20000 噪声流旧成员淡出+重复插入持续可见+双实例一致钉住；淡出可观测性由噪声流/槽数比调参钉住）；与 CountingBloomFilter（6036）同族不同面：显式删除 vs 隐式时间衰减（spec 6037） | [spec 6037](docs/spec/6037-stable-bloom.md) |
| 策略治理 | Weighted Reservoir Sampler 加权蓄水池采样 | WeightedReservoirSampler——A-Chao 单遍按权重入样的加权蓄水池（A-Chao 思想）：前 m 项直接入池，其后第 i 项以 w_i/sumW 概率替换池中均匀随机一项（替换成功才累计 sumWeight）；种子化 Random（同种子同流同样本——确定性可回放）+sample/size/seenCount/capacity 读数+capacity≤0/weight≤0 fail-fast——先收集全部再抽（内存随流线性爆）的病解（500 项池 10 守恒+百万权重项驻留+同种子双实例样本全等+欠容量全保留钉住）；与 ReservoirSample（observability）同族不同面：等权均匀 vs 权重入样概率（spec 6039） | [spec 6039](docs/spec/6039-weighted-reservoir.md) |
| 并发治理 | Median Finder 双堆中位数流 | MedianFinder——最大堆保左半+最小堆保右半夹逼的流式中位数（双堆经典思想）：插入经再平衡（两堆大小差≤1）O(log n)，中位数 O(1) 取（奇数取大堆顶、偶数取两顶无符号右移均值——奇偶显式两态防溢出）+size/isEmpty 读数+空返回 null（流未开始诚实）——每次查询全量排序 O(n log n) 放大的病解（500 随机流每步与排序圣像逐步全等+奇偶两态+固定流序列锚 3,2,3,2,3,3,3,3+MIN/0/MAX 极值钉住）；与 IndexedHeap（6026）同族不同面：单序堆+位置映射 vs 双堆夹逼中位数（spec 6040） | [spec 6040](docs/spec/6040-median-finder.md) |
| 限流治理 | AIMD Window 加性增/乘性减窗口 | AimdWindow——成败二值信号驱动的加性增/乘性减自适应窗口（TCP 拥塞避免思想）：成功 +1（钳 max）、失败 ×β 向上取整（钳 min）——乘性减快速退避释放压力、加性增缓慢探测余量（锯齿收敛经典）+current/minLimit/maxLimit/decreaseFactor 读数+min>max/min<1/β∉(0,1) fail-fast——固定阈值限流无视拥塞信号与纯加性调节收敛慢的病解（锯齿形态 18 增至 20→减半 10→再增 20 逐值钉住+取整与 min 钳制语义+双实例确定性）；与 GradientAdaptiveLimiter（concurrent）同族不同面：梯度延迟信号 vs 成败二值 AIMD 锯齿（spec 6041） | [spec 6041](docs/spec/6041-aimd-window.md) |
| 过程治理 | T 系 T36 周期对账 | Wave 6 五新类型快照补登（1233→1238：BuddyAllocator——memory + ExternalMergeSort——fs + ExtendibleHashing——metrics + ElevatorScan——policy + ArenaAllocator——memory）+ 全仓 16 模块 verify 三门绿（R48 协议口径）+ T 系第六波对账门（spec 6000–6034 零缺位）+ 72% 里程碑（spec 6035） | [spec 6035](docs/spec/6035-t-t36-reconciliation.md) |
| 指标治理 | MemTable 内存表 | MemTable——写路径的可变有序内存表与满表滚动（LSM memtable 思想）：put upsert（同键覆盖不增位）、表满拒写返回 false（滚动交接责任在调用方——drain 后可再写），drain 整表按 key 字典序导出并清空（滚动到不可变段的确定性交接点）+ isFull/size 读数 + null 键值/负容量 fail-fast——无限累积（内存爆炸）与满即阻塞（无滚动语义）的病解；与 SparseIndex（S22）同族不同面：可变写侧 vs 不可变读侧（spec 5024） | [spec 5024](docs/spec/5024-memtable.md) |
| 指标治理 | Skip List 跳跃表 | SkipList——有序动态集合的概率多层链（Pugh 1990/Redis zset 底层思想）：每键塔高由种子化随机发生器逐半衰（p=1/2 封顶 8 层），查找/插入期望 O(log n)；put upsert/get/keysInOrder（第 0 层顺序确定性）+ 种子注入（同种子同层高同结构）+ null 键值 fail-fast——有序数组插入 O(n) 与平衡树旋转复杂度的病解（固定种子 500 随机操作 vs TreeMap 圣像键序取值全等钉住）（spec 5025） | [spec 5025](docs/spec/5025-skip-list.md) |
| 策略治理 | 平滑加权轮询 | WeightedRoundRobin——加权分发的平滑插值（nginx smooth weighted round-robin 思想）：每轮各节点 current += weight、选 current 最大者发出并扣 totalWeight——权重节点请求均匀插散（{a:5,b:1,c:1} 七轮 = aabacaa，a 不扎堆），权重比精确；LinkedHashMap 注册序保并列 tie-break 确定性 + currents 读数 + 空表/weight≤0 fail-fast——朴素 WRR（aaaaa bc 短窗倾斜）的病解；与 DeficitRoundRobin（S28）同族不同面：权重平滑 vs 字节亏空（spec 5026） | [spec 5026](docs/spec/5026-weighted-round-robin.md) |
| 策略治理 | Deficit Round Robin 亏空调度 | DeficitRoundRobin——异构负载公平排队的亏空记账（Shreedhar-Varghese DRR 思想）：轮转各队 deficit += quantum，队头 size ≤ deficit 则发出并扣减；队空亏空清零跳过；亏空跨轮结转——小流量积少成多不被大包压死；队列字节容量守恒（满拒入队）+ 单轮无服务 null 确定性不阻塞 + deficits/queueBytes 读数 + 未知队列/size≤0 fail-fast——纯轮询按条数（大包小包同价——字节不公平）的病解；与 WeightedRoundRobin（S27）同族不同面：权重平滑 vs 字节亏空（spec 5027） | [spec 5027](docs/spec/5027-deficit-round-robin.md) |
| 恢复治理 | Segment Log 分段日志 | SegmentLog——追加日志的分段滚动与最旧段淘汰（Kafka 分段日志思想）：append 分配递增 LSN、段满（segmentCapacity 条）滚动新段、段数超 maxSegments 淘汰最旧段（droppedCount 保留上限诚实可见）；readAll 存活记录跨段按 LSN 序拼接 + firstSurvivingLsn/totalAppended 读数——单文件无限增长（清理只能整文件）与无保留上限（磁盘耗尽）的病解；与 SessionArchiver（归档）同族不同面：归档导出 vs 分段保留窗口（spec 5028） | [spec 5028](docs/spec/5028-segment-log.md) |
| 事务治理 | Read Repair 读修复 | ReadRepair——多副本读的读时择优与陈旧回写（Dynamo/Cassandra read repair 思想）：stitch 取 N 副本汇报中最高版本为胜者（读返回值来源），版本低于胜者者入陈旧清单（副本名字典序确定性排序）供调用方回写胜者值；版本并列副本名字典序最小者胜（确定性 tie-break）——读到旧版本不处理（撕裂持续）的病解；与 HintedHandoff（S11）同族不同面：读路径修复 vs 投递暂代（spec 5018） | [spec 5018](docs/spec/5018-read-repair.md) |
| 指标治理 | 热点 Key 探测器 | HotKeyDetector——热点 key 的确定性采样与阈值告警（Redis/网关热点 key 面思想）：record 按全局序号 mod sampleRate 命中采样一次计数（确定性、无随机——同访问序列同采样点），采样计数 ≥ hotThreshold 即热点；hotKeys 字典序全集 + sampledCountOf/globalSequence 读数 + 畸形定构/null key fail-fast——全量计数（内存随 key 基数爆炸）与拍脑袋告警（阈值无观测依据）的病解；与 CountMinSketch（R2 频次估计）互补：概率估计 vs 确定性采样告警（spec 5019） | [spec 5019](docs/spec/5019-hot-key-detector.md) |
| 背压治理 | Bounded Mailbox 有界信箱 | BoundedMailbox——消息接纳的容量信箱与显式溢出策略（Akka bounded mailbox 思想）：offer 未满入队、满按策略 DROP_NEWEST 拒新（false）/DROP_OLDEST 逐最旧纳新（droppedCount++ 且 true——新消息不因积压被拒）；poll FIFO + size/droppedCount/capacity 读数（积压与丢弃可见）——无界队列（积压不可见 OOM 隐患）与静默丢弃（丢了什么不知道）的病解；与 DisruptorRingBuffer（spec 5008）同族不同面：预分配环零分配 vs 策略性溢出信箱（spec 5020） | [spec 5020](docs/spec/5020-bounded-mailbox.md) |
| 指标治理 | Sparse Index 稀疏索引 | SparseIndex——排序块数据的块首键二分定位（LSM/SSTable sparse index 思想）：块（blockId, firstKey）升序注册（索引量级 O(块数)），locate 二分找最后一个 firstKey ≤ key 的块——只承诺范围不承诺存在（key 早于首块 -1 诚实不在）；乱序/空块集/null key fail-fast——每键全索引（索引与数据同量级）与全块线性扫（读放大）的病解；与 Bitcask 键目录（R15）同族不同面：全量内存目录 vs 稀疏块索引（spec 5021） | [spec 5021](docs/spec/5021-sparse-index.md) |
| 指标治理 | 区间树 stabbing 查询 | IntervalTree——区间重叠点查的居中分桶树（CLRS interval tree 思想）：区间按中点分桶递归建树——含中点者入本节点列表、全在左/右者下推子树；stabbing(point) 按点与中点关系走子树 + 节点列表过滤——O(log n+k) 命中所有包含该点的区间（结果 start,end 字典序确定性）；静态构建口径 + 倒置区间 fail-fast——全量线性扫（每查 O(n)）的病解（随机 60 区间 × 200 探针 vs 线性扫圣像全等钉住）；与 SweepLineIntervals 同族不同面：扫线全序列 vs 树 stabbing 点查（spec 5022） | [spec 5022](docs/spec/5022-interval-tree.md) |
| 过程治理 | S 系 S18 周期对账 | Wave 3 五新类型快照补登（1178→1183：DoubleWrite 双写缓冲/Group Commit 组提交——recovery×2 + 延迟调度——policy + SIEVE 缓存驱逐——cache + 2PC 协调器——transaction）+ 全仓 16 模块 verify 三门绿（R48 协议口径）+ S 系第三波对账门（spec 5000–5016 零缺位）+ 18/50=36% 里程碑（spec 5017） | [spec 5017](docs/spec/5017-s-s18-reconciliation.md) |
| 恢复治理 | Hinted Handoff 暂代投递 | HintedHandoff——目标暂不可用的不丢不阻塞记账回放（Cassandra hinted handoff 思想）：route 目标健康直投（false 无 hint）、下线转记 hint 入 FIFO 队（true——主路径不阻塞）；markDown/markUp 显式状态机（重复转换 IAE fail-fast），markUp 按 FIFO 回放并清队；恢复后再下线新 hint 独立累积 + hintCountOf/isDown 读数 + 未知目标 fail-fast——直接丢弃（恢复后数据缺口）与无休止阻塞重试（拖死主路径）的病解；与持久化 Outbox（effort #6）同族不同面：事件外发持久队列 vs 目标级暂代回放（spec 5010） | [spec 5010](docs/spec/5010-hinted-handoff.md) |
| 恢复治理 | DoubleWrite 双写缓冲 | DoubleWriteBuffer——页级写崩溃撕裂的恢复源（InnoDB doublewrite buffer 思想）：页写入先入共享暂存缓冲（同页覆盖——最新版胜），缓冲满自动整体落盘（清空 + flushCount++ 容量守恒）；flush 手动清队返回页集、recoverable 崩溃恢复视图（暂存中各页最新版，防御性副本）——就地更新写到一半崩溃（页撕裂半新半旧）时以此为恢复源的病解；与 HintedHandoff 同族不同面：投递暂代 vs 崩溃恢复暂存（spec 5012） | [spec 5012](docs/spec/5012-doublewrite-buffer.md) |
| 恢复治理 | Group Commit 组提交 | GroupCommitLog——顺序化持久的一次落盘合并（PostgreSQL/InnoDB group commit 思想）：append 进当前组分配严格递增 LSN，sync 一次持久化整组（GroupSync 区间 records/from/to），durableUpto 持久上沿单调不减（已落盘可证）；空组空同步上沿不变、sync 后新组 LSN 连续——每事务一次 fsync（吞吐被设备延迟钳死）与无持久上沿（落盘不可知）的病解；与 GroupCommitAccounting（core/fs）同族不同面：统计读数 vs LSN 日志分组持久面（spec 5013） | [spec 5013](docs/spec/5013-group-commit-log.md) |
| 策略治理 | 延迟调度 | DelayScheduling——数据本地性等待预算（Spark delay scheduling/locality wait 思想）：首选本地槽可用即 LAUNCH(preferred) 零浪费；不可用 WAIT 累计 skips，超 maxSkips 预算 → LAUNCH(ANY) 并重置（本地性换时效）；降级梯 PROCESS_LOCAL→NODE_LOCAL→RACK_LOCAL→ANY 逐轮放宽 + skipsUsed/relaxedTo 读数 + 轮次判定确定性（无时间依赖）——立即任意执行（远程拉数据带宽爆炸）与无限等本地槽（任务饿死）的病解；与 SpeculativeStragglerPolicy（R27）互补：本地性等待 vs 慢任务竞争（spec 5014） | [spec 5014](docs/spec/5014-delay-scheduling.md) |
| 过程治理 | S 系 S12 周期对账 | Wave 2 五新类型快照补登（1173→1178：票据锁/序号锁——concurrent×2 + Disruptor 环形缓冲——backpressure + Clock-Sweep 缓存驱逐——cache + Hinted Handoff——recovery）+ 全仓 16 模块 verify 三门绿（R48 协议口径）+ S 系第二波对账门（spec 5000–5010 零缺位）+ 12/50=24% 里程碑（spec 5011） | [spec 5011](docs/spec/5011-s-s12-reconciliation.md) |
...
| 性能基线 | 阶梯加压计划 | RampProfile——台阶内线性爬坡插值+末台阶后保持+totalDuration/peakTarget 声明——k6/Gatling ramping stages 语义，压测负载曲线先声明后执行采样点与档位一一对应（spec 1892） | [spec 1892](docs/spec/1892-ramp-profile.md) |
| 事务语义 | 幂等键判定面 | IdempotencyKeyGuard——decide 三态（FIRST 首见/REPLAY 同键同参/CONFLICT 同键异参=idempotency_error）+isExpired TTL 恰到期失效——Stripe 语义，复用旧键改参数的副作用错配物理拦住；与 501 Advisor 存储重放面互补（spec 1893） | [spec 1893](docs/spec/1893-idempotency-key-guard.md) |
...
| 健康治理 | 探测流量预算 | ProbeBudget——probeShare 探测 QPS/容量 QPS 占比+verdict 两态边界含上——SRE 健康检查预算惯例，「探活变压死」事前可见（spec 1912） | [spec 1912](docs/spec/1912-probe-budget.md) |
| 工程治理 | O 系 R114 对账轮 | 快照补登前置第十九例行（五类型 1116→1121）+ 全仓离线 verify 绿 + Wave 20 排程（spec 1913） | [spec 1913](docs/spec/1913-o-r114-reconciliation.md) |
