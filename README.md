...
| 溢出治理 | varint 编解码 | VarintCodec——字节流整数紧凑+有符号不顶格（Protobuf zigzag+LEB128 思想）：有符号 long 先 zigzag 交错映射无符号（0→0/−1→1/1→2——负小值 1 字节起步，直接 LEB128 负数 10 字节顶格病的根治），再 7 位/字节小端续位自界定（小值 1 字节、值域每 7 位加一字节：300→2/2⁴⁰→7/极值 10）——键值对/计数器/时间戳等小值占多数的字节流地基；decodeAt 游标连解+截断/残留/超宽 fail-fast——与 EliasGamma（位级前缀）成编码双件按介质粒度选型（spec 3039） | [spec 3039](docs/spec/3039-varint-codec.md) |
