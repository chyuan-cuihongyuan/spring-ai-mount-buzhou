...
| 策略治理 | Jump Hash 跳跃一致哈希 | JumpHash——极简内存一致哈希的跳跃随机游走（Google Lamping-Veach 论文思想）：bucketOf 线性同余逐桶随机游走，桶数 m 增至 m+1 时约 m/(m+1) 键原桶不动（单调稳定——扩容只搬最少量的键）；FNV-1a 64 稳定指纹字符串入口（跨进程同键同桶）+ bucketCount≤0/null fail-fast——哈希环+虚拟节点（建环/重排开销）与朴素取模（扩容全量迁移）的病解；与 BoundedLoadRing（S4）同族不同面：极简无状态跳跃 vs 有界负载环（spec 5030） | [spec 5030](docs/spec/5030-jump-hash.md) |
